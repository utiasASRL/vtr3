...
void MetricLocalizationPipeline::localizeKeyFrame(QueryCachePtr q_data,
  ...
  *loc_data->map_id = *m_data->map_id;
  ...
  if (*loc_data->steam_failure == true || *loc_data->success == false) {
  ...
  auto edge_id =
      EdgeId(*q_data->live_id, *m_data->map_id, asrl::pose_graph::Spatial);
  asrl::pose_graph::RCEdge::Ptr edge;
