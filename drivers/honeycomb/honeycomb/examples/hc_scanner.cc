...
int main(int argc, char** argv) {
  ...
  std::vector<waymo::Honeycomb::DeviceAddress> devices;

  if ((status = lidar.ScanForDevices(&devices)) != waymo::Status::kOk) {
    std::cerr << "Failed to find any Honeycomb lidar: " << status << std::endl;
    return 1;
  }

  std::cout << "Scan returned " << devices.size() << " devices:" << std::endl;
  for (const auto& device : devices) {
    std::cout << "  ip = " << device.ip_address
              << ", mac = " << device.mac_address << std::endl;
  }
