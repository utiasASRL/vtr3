...
class MultiRobotGraphMap extends React.Component {
  // Store last map click position for robot selection
  lastMapClickLatLng = null;
  constructor(props) {
    super(props);
    // Multi-robot support: get robot IDs from props or arguments
    console.log("Constructing MultiRobotGraphMap with robot IDs:", props.robotIds);
  ...
  componentDidMount() {
    ...
    this.state.robotIds.forEach(id => {
      ...
      this.props.socket.on(`robot/state/${id}`, (state) => this.robotStateCallback(state, id));
      this.props.socket.on(`mission/server_state/${id}`, (state) => this.serverStateCallback(state, id));
    });
  }
  ...
  componentWillUnmount() {
    ...
    this.state.robotIds.forEach(id => {
      ...
      this.props.socket.off(`mission/server_state/${id}`);
    });
  }
  ...
  mapCreatedCallback(map) {
    ...
    //
    this.map.on("click", (e) => {
      this.lastMapClickLatLng = e.latlng;
  ...
  fetchGraphState(center = false) {
    ...
    .then((response) => {
      ...
      response.json().then((data) => {
        ...
        // fetch robot and following route after graph has been set for each robot
  ...
  // Fetch robot state, following route, and server state for all robots
  fetchRobotState() {
    this.state.robotIds.forEach(id => {
  ...
  fetchFollowingRoute() {
    this.state.robotIds.forEach(id => {
      fetchWithTimeout(`/vtr/following_route/${id}`)
        .then((response) => {
          if (response.status !== 200) throw new Error(`Failed to fetch following route for ${id}: ` + response.status);
          response.json().then((data) => {
            this.loadFollowingRoute(data, id);
          });
        })
        .catch((error) => {
          console.error(error);
  ...
  // Update callbacks to handle per-robot data
  followingRouteCallback(route, robot_id) {
    this.loadFollowingRoute(route, robot_id);
  }

  loadFollowingRoute(route, robot_id) {
  ...
  robotStateCallback(state, robot_id) {
    this.loadRobotState(state, robot_id);
  }
  ...
  loadRobotState(state, robot_id) {
    ...
    if (state.valid === false) {
      if (markerObj.valid === true) {
        markerObj.valid = false;
        markerObj.marker.remove();
      }
    } else {
      markerObj.marker.setLatLng({ lng: state.lng, lat: state.lat });
      markerObj.marker.setOpacity(state.localized ? ROBOT_OPACITY : ROBOT_UNLOCALIZED_OPACITY);
      markerObj.marker.setRotationAngle(-(state.theta / Math.PI) * 180);
      if (markerObj.valid === false) {
        markerObj.valid = true;
        markerObj.marker.addTo(this.map);
      }
    }
    // Target marker logic
    const targetObj = this.state.targetMarkers[robot_id];
    if (!targetObj) return;
    if (state.target_valid === false) {
      if (targetObj.valid === true) {
        targetObj.valid = false;
        targetObj.marker.remove();
      }
    } else {
      targetObj.marker.setLatLng({ lng: state.target_lng, lat: state.target_lat });
      targetObj.marker.setOpacity(state.target_localized ? ROBOT_OPACITY : ROBOT_UNLOCALIZED_OPACITY);
      targetObj.marker.setRotationAngle(-(state.target_theta / Math.PI) * 180);
      if (targetObj.valid === false) {
        targetObj.valid = true;
        targetObj.marker.addTo(this.map);
      }
    }
  }

  // Fetch server state for all robots
  fetchServerState() {
    this.state.robotIds.forEach(id => {
      this._fetchServerStateForRobot(id);
    });
  }

  // Private helper to fetch server state for a single robot
  _fetchServerStateForRobot(robot_id) {
    console.info(`Fetching the current server state for robot ${robot_id}.`);
    fetchWithTimeout(`/vtr/server/${robot_id}`)
      .then((response) => {
        if (response.status !== 200) throw new Error(`Failed to fetch server state for ${robot_id}: ` + response.status);
        response.json().then((data) => {
          console.info(`Received the server state for robot ${robot_id}: `, data);
          this.loadServerState(data, robot_id);
        });
      })
      .catch((error) => {
        console.error(error);
  ...
  serverStateCallback(state, robot_id) {
    this.loadServerState(state, robot_id);
  }

  loadServerState(state, robot_id) {
    console.info("Loading the current server state: ", state);
    let curr_goal_idx = -1;
    for (let i = 0; i < state.goals.length; i++) {
      if (state.goals[i].id.toString() === state.current_goal_id.toString()) {
        curr_goal_idx = i;
        break;
      }
    }
...
export default MultiRobotGraphMap;