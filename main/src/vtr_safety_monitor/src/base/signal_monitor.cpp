...
SignalMonitor::SignalMonitor(const std::shared_ptr<rclcpp::Node> node)
    : node_(node) {
  desired_action = PAUSE;
  signal_monitor_status_unknown = true;
  max_allowed_speed_ =
      node_->get_parameter("max_allowed_speed").as_double();  // todo: prefix?
}
...
int SignalMonitor::getDesiredAction() { return desired_action; }

void SignalMonitor::setStatusUnknown() { signal_monitor_status_unknown = true; }
...
rclcpp::Time SignalMonitor::getLastUpdateTime() { return last_update; }

void SignalMonitor::setMsgTimeout(double new_msg_timeout) {
  max_time_between_updates = new_msg_timeout;
}

double SignalMonitor::getMaxAllowedSpeed() { return max_allowed_speed_; }
