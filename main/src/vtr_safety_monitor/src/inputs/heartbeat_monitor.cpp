...
HeartbeatMonitorInput::HeartbeatMonitorInput(
  ...
  status_subscriber_ = node_->create_subscription<RobotStatus>(
      "robot", 10,
      std::bind(&HeartbeatMonitorInput::statusCallback, this,
                std::placeholders::_1));
  period_ = node_->declare_parameter<double>("heartbeat_period", 2.0);
  timer_ = rclcpp::create_timer(
      node_, node_->get_clock(),
      std::chrono::milliseconds((long)(1000.0 * period_)),
      std::bind(&HeartbeatMonitorInput::timedCallback, this));

  LOG(INFO) << "Heartbeat period: " << period_;
}

void HeartbeatMonitorInput::statusCallback(
