...
TEST_F(StorageTestFixture, read_next_returns_filtered_messages) {
  ...
  // Test reset filter
  std::unique_ptr<ReadOnlyInterface> readable_storage2 =
    std::make_unique<sqlite::SqliteStorage>();
...
TEST_F(StorageTestFixture, get_all_topics_and_types_returns_the_correct_vector) {
  ...
  auto readable_storage = std::make_unique<sqlite::SqliteStorage>();
  readable_storage->open(read_only_filename, IOFlag::READ_ONLY);
  auto topics_and_types = readable_storage->get_all_topics_and_types();

  EXPECT_THAT(
    topics_and_types, ElementsAreArray(
  {
    TopicMetadata{"topic1", "type1", "rmw1", ""},
    TopicMetadata{"topic2", "type2", "rmw2", ""}
  }));
}
...
TEST_F(StorageTestFixture, get_metadata_returns_correct_struct) {
  ...
  const auto readable_storage = std::make_unique<sqlite::SqliteStorage>();
  const auto db_filename = (rcpputils::fs::path(temporary_dir_path_) / "rosbag.db3").string();

  readable_storage->open(db_filename, IOFlag::READ_ONLY);
  ...
  {
    TopicInformation{TopicMetadata{
...
TEST_F(StorageTestFixture, get_storage_identifier_returns_sqlite3) {
  const auto storage = std::make_unique<sqlite::SqliteStorage>();
...
TEST_F(StorageTestFixture, get_relative_file_path_returns_db_name_with_ext) {
  ...
  const auto read_write_storage = std::make_unique<sqlite::SqliteStorage>();
  read_write_storage->open(read_write_filename, IOFlag::READ_WRITE);
  ...
  const auto read_only_storage = std::make_unique<sqlite::SqliteStorage>();
  read_only_storage->open(read_only_filename, IOFlag::READ_ONLY);
  EXPECT_EQ(read_only_storage->get_relative_file_path(), storage_filename);

  const auto & append_filename = storage_filename;
  const auto append_storage = std::make_unique<sqlite::SqliteStorage>();
  append_storage->open(append_filename, IOFlag::APPEND);
