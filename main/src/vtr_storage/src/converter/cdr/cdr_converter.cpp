...
{
  introspection_message_set_topic_name(
  ...
  if (ret != RMW_RET_OK) {
    throw std::runtime_error{"ERROR [storage] Failed to deserialize message."};
  }
}
  ...
  std::shared_ptr<SerializedBagMessage> serialized_message)
{
  ...
  if (ret != RMW_RET_OK) {
    throw std::runtime_error{"ERROR [storage] Failed to serialize message."};
  }
}
// clang-format on
