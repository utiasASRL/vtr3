...
#include <vtr_pose_graph/rc_graph/rc_edge.hpp>
#include <vtr_pose_graph/rc_graph/rc_run.hpp>
#include <vtr_pose_graph/rc_graph/rc_vertex.hpp>
...
class RCGraphBase : public virtual GraphBase<RCVertex, RCEdge, RCRun> {
 public:
  ...
  using PersistentIdMsg = vtr_pose_graph_msgs::msg::PersistentId;
  ...
  PersistentIdMsg toPersistent(const VertexIdType& vid) const;

  // Get the vertex id from persistent id (unchanged on graph refactor)
  VertexIdType fromPersistent(const PersistentIdMsg& pid) const;
 ...
 protected:
  using PersistentMap = std::unordered_map<PersistentIdMsg, VertexIdType,
                                           PersistentIdHasher>;
