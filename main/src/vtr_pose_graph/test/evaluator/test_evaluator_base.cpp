...
TEST_F(EvaluatorTestFixture, WeightMap) {
  ...
  for (auto it = vertices.begin(); it != vertices.end(); ++it) {
    eval->ref(it->first) = it->second->id().minorId() + 10.f * it->second->id().majorId();
  }
  for (auto it = edges.begin(); it != edges.end(); ++it) {
    eval->ref(it->first) = it->second->id().minorId1() + it->second->id().minorId2() + 10.f * it->second->id().majorId2();
  }

  // Check using both GraphId and SimpleId
  for (auto it = vertices.begin(); it != vertices.end(); ++it) {
    EXPECT_EQ(eval->at(it->first), it->second->id().minorId() + 10.f * it->second->id().majorId());
    EXPECT_EQ(eval->at(it->second->id()), it->second->id().minorId() + 10.f * it->second->id().majorId());
    EXPECT_EQ((*eval)[it->first], it->second->id().minorId() + 10.f * it->second->id().majorId());
    EXPECT_EQ((*eval)[it->second->id()], it->second->id().minorId() + 10.f * it->second->id().majorId());
  }
  for (auto it = edges.begin(); it != edges.end(); ++it) {
    EXPECT_EQ(eval->at(it->first), (it->second->id().minorId1() + it->second->id().minorId2() + 10.f * it->second->id().majorId2()));
    EXPECT_EQ(eval->at(it->second->id()), (it->second->id().minorId1() + it->second->id().minorId2() + 10.f * it->second->id().majorId2()));
    EXPECT_EQ((*eval)[it->first], (it->second->id().minorId1() + it->second->id().minorId2() + 10.f * it->second->id().majorId2()));
    EXPECT_EQ((*eval)[it->second->id()], (it->second->id().minorId1() + it->second->id().minorId2() + 10.f * it->second->id().majorId2()));
  }

  for (auto it = vertices.begin(); it != vertices.end(); ++it) {
    eval->ref(it->second->id()) = 17.f * (it->second->id().minorId() + 10.f * it->second->id().majorId());
  }

  for (auto it = edges.begin(); it != edges.end(); ++it) {
    eval->ref(it->second->id()) = 17.f * (it->second->id().minorId1() + it->second->id().minorId2() + 10.f * it->second->id().majorId2());
  }

  for (auto it = vertices.begin(); it != vertices.end(); ++it) {
    EXPECT_EQ(eval->at(it->first), (17.f * (it->second->id().minorId() + 10.f * it->second->id().majorId())));
    EXPECT_EQ(eval->at(it->second->id()), (17.f * (it->second->id().minorId() + 10.f * it->second->id().majorId())));
    EXPECT_EQ((*eval)[it->first], (17.f * (it->second->id().minorId() + 10.f * it->second->id().majorId())));
    EXPECT_EQ((*eval)[it->second->id()], (17.f * (it->second->id().minorId() + 10.f * it->second->id().majorId())));
  }

  for (auto it = edges.begin(); it != edges.end(); ++it) {
    EXPECT_EQ(eval->at(it->first), (17.f * (it->second->id().minorId1() + it->second->id().minorId2() + 10.f * it->second->id().majorId2())));
    EXPECT_EQ(eval->at(it->second->id()), (17.f * (it->second->id().minorId1() + it->second->id().minorId2() + 10.f * it->second->id().majorId2())));
    EXPECT_EQ((*eval)[it->first], (17.f * (it->second->id().minorId1() + it->second->id().minorId2() + 10.f * it->second->id().majorId2())));
    EXPECT_EQ((*eval)[it->second->id()], (17.f * (it->second->id().minorId1() + it->second->id().minorId2() + 10.f * it->second->id().majorId2())));
  }
}
...
TEST_F(EvaluatorTestFixture, WeightBasicMath) {
  ...
  for (auto it = edges.begin(); it != edges.end(); ++it) {
    ...
    EXPECT_NEAR(n1->at(it->first), -res, 1e-10);
    EXPECT_NEAR(QQ->at(it->first), -res * (offset + res) / (offset - res), 1e-6);
  }
}
...
TEST_F(EvaluatorTestFixture, WeightComparison) {
  ...
  double v11 = eval1->at(vertices.begin()->first);
  double e11 = eval1->at(edges.begin()->first);
...
TEST_F(EvaluatorTestFixture, MaskMap) {
  ...
  // EVEN
  for (auto it = vertices.begin(); it != vertices.end(); ++it) {
    eval->ref(it->first) = (it->second->id().minorId() + it->second->id().majorId()) % 2 == 0;
  }
  for (auto it = edges.begin(); it != edges.end(); ++it) {
    eval->ref(it->first) = (((it->second->id().minorId1() + it->second->id().minorId2() + it->second->id().majorId2()) % 2) == 0);
  }

  // Check using both GraphId and SimpleId
  for (auto it = vertices.begin(); it != vertices.end();
       ++it) {
    EXPECT_EQ(eval->at(it->first),((it->second->id().minorId() + it->second->id().majorId()) % 2 == 0));
    EXPECT_EQ(eval->at(it->second->id()),((it->second->id().minorId() + it->second->id().majorId()) % 2 == 0));
    EXPECT_EQ((*eval)[it->first],((it->second->id().minorId() + it->second->id().majorId()) % 2 == 0));
    EXPECT_EQ((*eval)[it->second->id()],((it->second->id().minorId() + it->second->id().majorId()) % 2 == 0));
  }
  for (auto it = edges.begin(); it != edges.end(); ++it) {
    EXPECT_EQ(eval->at(it->first), ((it->second->id().minorId1() + it->second->id().minorId2() + it->second->id().majorId2()) % 2 == 0));
    EXPECT_EQ(eval->at(it->second->id()), ((it->second->id().minorId1() + it->second->id().minorId2() + it->second->id().majorId2()) % 2 == 0));
    EXPECT_EQ((*eval)[it->first], ((it->second->id().minorId1() + it->second->id().minorId2() + it->second->id().majorId2()) % 2 == 0));
    EXPECT_EQ((*eval)[it->second->id()], ((it->second->id().minorId1() + it->second->id().minorId2() + it->second->id().majorId2()) % 2 == 0));
  }

  // Assign masks based on a pattern, using VertexId/EdgeId: true if sum of
  // minor and major ids is ODD
  for (auto it = vertices.begin(); it != vertices.end(); ++it) {
    eval->ref(it->second->id()) = (it->second->id().minorId() + it->second->id().majorId()) % 2 != 0;
  }
  for (auto it = edges.begin(); it != edges.end(); ++it) {
    eval->ref(it->second->id()) = ((it->second->id().minorId1() + it->second->id().minorId2() + it->second->id().majorId2()) % 2) != 0;
  }
...
TEST_F(EvaluatorTestFixture, MaskComparison) {
  auto drnd = std::bind(std::uniform_real_distribution<double>{0, 100}, std::mt19937(std::random_device{}()));
