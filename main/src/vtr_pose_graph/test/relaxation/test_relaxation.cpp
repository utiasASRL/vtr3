...
class RelaxationTest : public Test {
 protected:
  RelaxationTest()
  ...
  void SetUp() override {
    ...
    for (int idx = 0; idx < GRAPH_SIZE; ++idx) {
      ...
      for (int idz = 1; idz < GRAPH_SIZE; ++idz) {
        graph_->addVertex();
        auto xEdge = graph_->addEdge(VertexId(idx, idz), VertexId(idx, idz - 1),
                                     Temporal);
      ...
      for (int idy = 1; idy < GRAPH_SIZE; ++idy) {
        graph_->addVertex();
        auto yEdge =
            graph_->addEdge(VertexId(idx, idy * GRAPH_SIZE),
                            VertexId(idx, (idy - 1) * GRAPH_SIZE), Temporal);
        yEdge->setTransform(randomTf() * Ty);

        for (int idz = 1; idz < GRAPH_SIZE; ++idz) {
          graph_->addVertex();
          auto yEdge = graph_->addEdge(
              VertexId(idx, idy * GRAPH_SIZE + idz),
              VertexId(idx, (idy - 1) * GRAPH_SIZE + idz), Temporal);
          auto xEdge = graph_->addEdge(
              VertexId(idx, idy * GRAPH_SIZE + idz),
              VertexId(idx, idy * GRAPH_SIZE + idz - 1), Temporal);
    ...
    for (int idx = 1; idx < GRAPH_SIZE; ++idx) {
      for (int idy = 0; idy < GRAPH_SIZE * GRAPH_SIZE; ++idy) {
        auto zEdge = graph_->addEdge(VertexId(idx, idy), VertexId(idx - 1, idy),
                                     Temporal);
...
class STRelaxationTest : public Test {
 protected:
  ...
  void SetUp() override {
    ...
    for (int idy = 1; idy < runLen_; ++idy) {
      graph_->addVertex();
      auto tmpEdge =
          graph_->addEdge(VertexId(0, idy), VertexId(0, idy - 1), Temporal);
    ...
    for (int idx = 1; idx < numRuns_; ++idx) {
      ...
      for (int idy = 1; idy < runLen_; ++idy) {
        graph_->addVertex();
        auto tmpEdge = graph_->addEdge(VertexId(idx, idy),
                                       VertexId(idx, idy - 1), Temporal);
        tmpEdge->setTransform(randomTf() * T);

        tmpEdge =
            graph_->addEdge(VertexId(idx, idy), VertexId(0, idy), Temporal);
