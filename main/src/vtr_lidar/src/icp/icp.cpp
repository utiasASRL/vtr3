...
void regu_pose_cycle(vector<Eigen::Matrix4d>& H, vector<float>& H_w) {
  ...
  //
  // H[i] = H(i,i-1) transformation that alignes frame i on frame i-1 (i is
  // modulo B) Example for B = 5 We Have H04 H10 H21 H32 H43 For H10 we compute
  // H_bis = H40 * H34 * H23 * H12 which we want equal to H10 Therefore newH10 =
  // pose_interp(w1, H10, H_bis, 0) To regularize all poses together, all wi =
  // 1/B. But if you favor one transformation (the largest for example). Change
  // w

  // Ensure sum of w equals 1
  float reg_w = 1.0 / std::accumulate(H_w.begin(), H_w.end(), 0);
  for (auto& w : H_w) w *= reg_w;
...
void SolvePoint2PlaneLinearSystem(const Matrix6d& A, const Vector6d& b,
  ...
  if (!Aqr.isInvertible()) {
    ...
    Q2] and R = [ R1 ; R2 ] such that ||R2|| is small (or zero) and therefore A
    = QR ~= Q1 * R1 const int rank = Aqr.rank(); const int rows = A.rows();
    ...
    if (findMinimalNormSolution) {
            // by getting the solution of smallest norm (x = R1^T * (R1 *
    R1^T)^-1 Q1^T b. x = R1.template triangularView<Eigen::Upper>().transpose()
    * (R1 * R1.transpose()).llt().solve(Q1t * b);
    }
    else {
            // by solving the simplest problem that yields fewest nonzero
    components in x x.block(0, 0, rank, 1) = R1.block(0, 0, rank, rank).template
    triangularView<Eigen::Upper>().solve(Q1t * b); x.block(rank, 0, rows - rank,
    1).setZero();
    }
    ...
    if (!b.isApprox(ax, 1e-5)) {
            LOG_INFO_STREAM("PointMatcher::icp - encountered almost singular
    matrix while minimizing point to plane distance. QR solution was too
    inaccurate. Trying more accurate approach using double precision SVD."); x =
    A.template cast<double>().jacobiSvd(ComputeThinU |
    ComputeThinV).solve(b.template cast<double>()).template cast<T>(); ax = A *
    x;

            if ((b - ax).norm() > 1e-5 * std::max(A.norm() * x.norm(),
    b.norm())) { LOG_WARNING_STREAM("PointMatcher::icp - encountered numerically
    singular matrix while minimizing point to plane distance and the current
...
void BundleICP(vector<PointXYZ>& points, vector<PointXYZ>& normals,
  ...
  for (size_t b = 0; b < B; b++) {
    refClouds[b].pts =
        vector<PointXYZ>(points.begin() + p0, points.begin() + p0 + lengths[b]);
    p0 += lengths[b];
  }
  ...
  // Build KDTrees
  for (auto& index : KDTrees) index->buildIndex();
  ...
  for (auto& cloud : refClouds)
    refMats.push_back(Eigen::Map<Eigen::Matrix<float, 3, Eigen::Dynamic>>(
        (float*)cloud.pts.data(), 3, cloud.pts.size()));

  // Matrix for aligned data (Shallow copy of parts of the stacked "points"
  // vector)
  p0 = 0;
  vector<Eigen::Map<Eigen::Matrix<float, 3, Eigen::Dynamic>>> alignedMats;
  for (size_t b = 0; b < B; b++) {
    alignedMats.push_back(Eigen::Map<Eigen::Matrix<float, 3, Eigen::Dynamic>>(
        (float*)(points.data() + p0), 3, lengths[b]));
    p0 += lengths[b];
  }
  ...
  if (W == 1) {
    ...
    p0 = 0;
    for (size_t b = 0; b < B; b++) {
      distributions.push_back(discrete_distribution<int>(
          weights.begin() + p0, weights.begin() + p0 + lengths[b]));
      p0 += lengths[b];
    }
  } else {
    ...
    for (size_t b = 0; b < B; b++) {
      for (size_t w = 0; w < W; w++) {
        ...
        stable_sort(best_idx[b + w * B].begin(), best_idx[b + w * B].end(),
                    [&weights, &w, &W](size_t i1, size_t i2) {
                      return weights[w + i1 * W] > weights[w + i2 * W];
                    });
      }
      p0 += lengths[b];
    }
  ...
  for (auto& frame_rms : results.all_rms) frame_rms.reserve(params.max_iter);
  ...
  for (size_t step = 0; step < max_it; step++) {
    ...
    // Only one weight means random sampling. We compute best indices at each
    // iteration
    if (W == 1) {
      p0 = 0;
      for (size_t b = 0; b < B; b++) {
    ...
    for (size_t b = 0; b < B; b++) {
      ...
      int tree_p0 = 0;
      for (size_t i = 0; i < tree_b; i++) tree_p0 += lengths[i];
      ...
      for (size_t w = 0; w < W; w++) {
        ...
        size_t i = 0;
        while (best_sample < best_idx[b + w * B].size() &&
               i < params.n_samples) {
          // Get next best idx
          ...
          // Get its neighbor
          KDTrees[tree_b]->knnSearch((float*)&points[candidate_ind], 1,
                                     &candidate_neighb[0], &candidate_dist[0]);

          // Verify it is in range
          // PointXYZ diff = (points[all_sample_inds[b][i].second] -
          // points[all_sample_inds[b][i].first]);
          // diff.dot(normals[all_sample_inds[b][i].second])
          if (candidate_dist[0] <
              params.max_pairing_dist * params.max_pairing_dist) {
            i++;
            PointXYZ diff = (references[candidate_neighb[0] + tree_p0] -
                             points[candidate_ind]);
            float planar_dist =
                abs(diff.dot(normals[candidate_neighb[0] + tree_p0]));
            rms2 += planar_dist;
            all_sample_inds[b].push_back(pair<size_t, size_t>(
                candidate_ind, candidate_neighb[0] + tree_p0));
          }
        }
      }

      // Compute RMS
      results.all_rms[b].push_back(
          sqrt(rms2 / (float)all_sample_inds[b].size()));
    }
    ...
    // Minimize error
    for (size_t b = 0; b < B; b++) {
      // Do not use weights in minimization, because we used them for random
      // sampling
      vector<float> ones(weights.size(), 1.0);
      PointToPlaneErrorMinimizer(points, references, normals, ones,
                                 all_sample_inds[b], H_icp[b]);
    }
    ...
    for (size_t b = 0; b < B; b++) {
      Eigen::Matrix3f R_tot =
          (results.transforms[b].block(0, 0, 3, 3)).cast<float>();
      Eigen::Vector3f T_tot =
          (results.transforms[b].block(0, 3, 3, 1)).cast<float>();
    ...
    if (step == 0) {
      ...
      for (size_t b = 0; b < B; b++)
        new_H.block(0, b * 4, 4, 4) = Eigen::MatrixXd(results.transforms[b]);
      Eigen::MatrixXd temp(results.all_transforms.rows() + 4,
                           results.all_transforms.cols());
      ...
      if (step > 0)
        for (size_t b = 0; b < B; b++) {
          ...
          Eigen::Matrix3d R2 = results.all_transforms.block(
              results.all_transforms.rows() - 4, b * 4, 3, 3);
          Eigen::Matrix3d R1 = results.all_transforms.block(
              results.all_transforms.rows() - 8, b * 4, 3, 3);
          Eigen::Vector3d T2 = results.all_transforms.block(
              results.all_transforms.rows() - 4, b * 4 + 3, 3, 1);
          Eigen::Vector3d T1 = results.all_transforms.block(
              results.all_transforms.rows() - 8, b * 4 + 3, 3, 1);
          R1 = R2 * R1.transpose();
          T1 = -R1 * T1 + T2;
          float dT_b = T1.norm();
      ...
      for (size_t b = 0; b < B; b++) {
        ...
        for (size_t s = 0; s < params.avg_steps; s++) {
          Eigen::Matrix4d H = results.all_transforms.block(
              results.all_transforms.rows() - 4 * (1 + s), b * 4, 4, 4);
          mH = pose_interp(1.0 / (float)(1 + s), mH, H, 0);
        }
        results.transforms[b] = mH;
        results.all_transforms.block(results.all_transforms.rows() - 4, b * 4,
                                     4, 4) = mH;
      }
    }
...
void PointToMapICPDebug(vector<PointXYZ>& tgt_pts, vector<float>& tgt_w,
  ...
  if (params.motion_distortion) {
    ...
    for (auto& p : tgt_pts) {
      ...
      if (phis.back() > phi1) phi1 = phis.back();
    }
  }
  ...
  Eigen::Map<Eigen::Matrix<float, 3, Eigen::Dynamic>> map_mat(
      (float*)map_points.data(), 3, map_points.size());
  ...
  for (size_t step = 0; step < max_it; step++) {
    ...
    for (size_t i = 0; i < sample_inds.size(); i++)
      map_tree.knnSearch((float*)&aligned[sample_inds[i].first], 1,
                         &sample_inds[i].second, &nn_dists[i]);
    ...
    for (size_t i = 0; i < sample_inds.size(); i++) {
      if (nn_dists[i] < params.max_pairing_dist) {
        ...
        // update pt2pl rms
        PointXYZ diff =
            (map_points[sample_inds[i].second] - aligned[sample_inds[i].first]);
    ...
    // Minimize error
    PointToPlaneErrorMinimizer(aligned, map_points, map_normals, map_scores,
    ...
    if (params.motion_distortion) {
      ...
      for (auto& phi : phis) {
        float t = (phi - params.init_phi) / (phi1 - params.init_phi);
        Eigen::Matrix4d phi_H =
            pose_interp(t, params.init_transform, results.transform, 0);
      ...
      Eigen::Matrix3f R_tot =
          (results.transform.block(0, 0, 3, 3)).cast<float>();
      Eigen::Vector3f T_tot =
          (results.transform.block(0, 3, 3, 1)).cast<float>();
...
void PointToMapICP(vector<PointXYZ>& tgt_pts, vector<float>& tgt_w,
  ...
  for (size_t step = 0; step < max_it; step++) {
    ...
    // Minimize error
    PointToPlaneErrorMinimizer(aligned, map.cloud.pts, map.normals, map.scores,
                               filtered_sample_inds, H_icp);
