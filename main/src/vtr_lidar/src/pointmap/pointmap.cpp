...
void PointMapPython::update(vector<PointXYZ>& points0,
  ...
  if (points.size() > 0) {
    ...
    PointXYZ oldOriginCorner =
        floor(oldMinCorner * (1 / dl) - PointXYZ(1, 1, 1)) * dl;
  ...
  // std::cout << "current max_load_factor: " << samples.max_load_factor() <<
  // std::endl; std::cout << "current size: " << samples.size() << std::endl;
  // std::cout << "current bucket_count: " << samples.bucket_count() <<
  // std::endl; std::cout << "current load_factor: " << samples.load_factor() <<
  // std::endl;
...
void PointMapPython::init_samples(
    const PointXYZ originCorner, const PointXYZ maxCorner,
    unordered_map<size_t, MapVoxelData>& samples) {
  // Dimensions of the grid
  size_t sampleNX = (size_t)floor((maxCorner.x - originCorner.x) / dl) + 2;
  size_t sampleNY = (size_t)floor((maxCorner.y - originCorner.y) / dl) + 2;
  ...
  for (auto& p : points) {
    ...
    samples.emplace(mapIdx, MapVoxelData(p * counts[i], normals[i] * counts[i],
                                         scores[i], counts[i]));
    i++;
  }
}
