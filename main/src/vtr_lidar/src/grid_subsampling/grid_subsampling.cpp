  ...
  for (auto& p : original_points) {
    ...
    if (samples.count(mapIdx) < 1) {
      samples.emplace(
          mapIdx,
          SampledCenter(i, p,
                        PointXYZ(originCorner.x + (iX + 0.5) * sampleDl,
...
void grid_subsampling_spheres(vector<PointXYZ>& original_points,
  ...
  PointXYZ originCorner =
      floor(minCorner * inv_dl - PointXYZ(1, 1, 1)) * sampleDl;

  // Dimensions of the grid
  size_t sampleNX =
      (size_t)floor((maxCorner.x - originCorner.x) / sampleDl) + 2;
  size_t sampleNY =
  ...
  for (auto& v : samples) {
    ...
    if (k0 == v.first) subsampled_points.push_back(centroid);
  }
  return;
}
...
void grid_subsampling(vector<PointXYZ>& original_points,
  ...
  // size_t sampleNZ = (size_t)floor((maxCorner.z - originCorner.z) / sampleDl)
  // + 1;
  ...
  for (auto& p : original_points) {
    ...
    if (data.count(mapIdx) < 1) data.emplace(mapIdx, SampledData(fdim, ldim));
...
void batch_grid_subsampling(
  ...
  for (b = 0; b < original_batches.size(); b++) {
    ...
    if (b_s_points.size() <= max_p) {
      subsampled_points.insert(subsampled_points.end(), b_s_points.begin(),
                               b_s_points.end());

      if (original_features.size() > 0)
        subsampled_features.insert(subsampled_features.end(),
                                   b_s_features.begin(), b_s_features.end());

      if (original_classes.size() > 0)
        subsampled_classes.insert(subsampled_classes.end(), b_s_classes.begin(),
                                  b_s_classes.end());
