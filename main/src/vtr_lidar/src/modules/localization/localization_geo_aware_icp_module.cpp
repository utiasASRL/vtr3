...
/**
 * \file localization_geo_aware_icp_module.cpp
 */

#include "vtr_lidar/modules/localization/localization_geo_aware_icp_module.hpp"
...
// data structure to hold pending cost terms 
struct CostTermData {
    Evaluable<Eigen::Matrix<double, 3, 1>>::ConstPtr error_func;
    StaticNoiseModel<3>::ConstPtr noise_model;
    Eigen::Vector3d nrm;  // Store normal for debugging if needed
    
    CostTermData(Evaluable<Eigen::Matrix<double, 3, 1>>::ConstPtr error_func, 
                 StaticNoiseModel<3>::ConstPtr noise_model, 
                 const Eigen::Vector3d& normal)
        : error_func(error_func), noise_model(noise_model), nrm(normal) {}
};

auto LocalizationGEOModule::Config::fromROS(const rclcpp::Node::SharedPtr &node,
  ...
  config->dratio_threshold = node->declare_parameter<float>(param_prefix + ".dratio_threshold", config->dratio_threshold);
...
void LocalizationGEOModule::run_(QueryCache &qdata0, OutputCache &output,
                                 const Graph::Ptr &,
  ...
  if(!*qdata.odo_success) {
    CLOG(WARNING, "lidar.localization_geo_aware_icp") << "Odometry failed, skip localization";
    return;
  }

  if (output.chain->isLocalized() && *qdata.loc_time > config_->target_loc_time && *qdata.pipeline_mode == tactic::PipelineMode::RepeatFollow) {
    CLOG(WARNING, "lidar.localization_geo_aware_icp") << "Skipping localization to save on compute. EMA val=" << *qdata.loc_time;
    return;
  }
  ...
  CLOG(DEBUG, "lidar.localization_geo_aware_icp") << "Start building a kd-tree of the map.";
  ...
  CLOG(DEBUG, "lidar.localization_geo_aware_icp") << "Start the ICP optimization loop.";
  for (int step = 0;; step++) {
    /// sample points
    timer[0]->start();
...
#pragma omp parallel for schedule(dynamic, 10) num_threads(config_->num_threads)
    ...
    // compute the scatter matrix to check if the point cloud is flat
    Eigen::Matrix3d Scatter = Eigen::Matrix3d::Zero();
    std::vector<CostTermData> pending_costs;
    pending_costs.reserve(filtered_sample_inds.size());

    // Use thread-private storage for scatter matrices
    std::vector<Eigen::Matrix3d> scatter_private(config_->num_threads, Eigen::Matrix3d::Zero());
    std::vector<std::vector<CostTermData>> costs_private(config_->num_threads);

    // cost terms and noise model
#pragma omp parallel for schedule(dynamic, 10) num_threads(config_->num_threads)
    for (const auto &ind : filtered_sample_inds) {
      // noise model for point-to-point
      // --- noise model W = n * n.T (information matrix)
      if (point_map[ind.second].normal_score <= 0.0) continue;
      Eigen::Vector3d nrm = map_normals_mat.block<3, 1>(0, ind.second).cast<double>();  // surface normal at the map ref. point

      int tid = omp_get_thread_num();    // get thread ID
      scatter_private[tid] += nrm * nrm.transpose();
      ...
      // point-to-point error evaluator
    ...
    for (int tid = 0; tid < config_->num_threads; tid++) {
        Scatter += scatter_private[tid];
        
        // Merge cost data
        pending_costs.insert(pending_costs.end(), 
                            costs_private[tid].begin(), 
                            costs_private[tid].end());
    }
    ...
    if (eigen_solver.info() != Eigen::Success) {
      CLOG(INFO, "lidar.localization_geo_aware_icp") << "Eigenvalue decomposition failed!!!!!!!!!!!!!!!!";
    } else {
      Eigen::Vector3d eigenvalues = eigen_solver.eigenvalues();
      // Eigenvalue ratios
      double eigenvalue_sum = eigenvalues.sum();
      if (eigenvalue_sum == 0) {
        CLOG(INFO, "lidar.localization_geo_aware_icp") << "Eigenvalue sum is zero, cannot compute ratios.";
        dominant_ratio = 1.0; // Avoid division by zero
      } else {
        // Calculate the ratio of the largest eigenvalue to the sum of eigenvalues
        dominant_ratio = eigenvalues(2) / eigenvalue_sum;     // largest eigenvalue
      }
      // double second_ratio = eigenvalues(1) / eigenvalue_sum;       
      // double smallest_ratio = eigenvalues(0) / eigenvalue_sum; 
      // Print results
      CLOG(INFO, "lidar.localization_geo_aware_icp") << "******************************************************************";
      CLOG(INFO, "lidar.localization_geo_aware_icp") << "Eigenvalues: [" 
          << eigenvalues(0) << ", " << eigenvalues(1) << ", " << eigenvalues(2) << "]";
      CLOG(INFO, "lidar.localization_geo_aware_icp") << "Dominant ratio: " << dominant_ratio;
    }


    // (2) Add all cost terms with adaptive loss
    // --- mapping function between dominant_ratio and cauchy_k param. 
    // double ada_cauchy_k = 0.0001 + 0.000009 * (1.0 - dominant_ratio) * (1.0 - dominant_ratio);
    // CLOG(INFO, "lidar.localization_geo_aware_icp") << "adaptive cauchy param: " << ada_cauchy_k;
    // auto loss_func = CauchyLossFunc::MakeShared(ada_cauchy_k); 

    auto loss_func = L2LossFunc::MakeShared(); 
    if (dominant_ratio < config_->dratio_threshold) {
      CLOG(DEBUG, "lidar.localization_geo_aware_icp") << "Adding " << pending_costs.size() << " cost terms with adaptive loss";

      for (const auto& cost_data : pending_costs) {
          auto cost = WeightedLeastSqCostTerm<3>::MakeShared(
              cost_data.error_func, 
              cost_data.noise_model, 
              loss_func);
    ...
    try{
      ...
      CLOG(WARNING, "lidar.localization_geo_aware_icp") <<  "Steam failed.\n e.what(): " << e.what();
      break;
    }
    ...
    if (!refinement_stage && step >= first_steps) {
      if ((step >= max_it - 1) || (mean_dT < config_->trans_diff_thresh &&
        ...
        CLOG(DEBUG, "lidar.localization_geo_aware_icp") << "Initial alignment takes " << step << " steps.";
    ...
    if ((refinement_stage && step >= max_it - 1) ||
      ...
      //
      CLOG(DEBUG, "lidar.localization_geo_aware_icp") << "Total number of steps: " << step << ", with matched ratio " << matched_points_ratio;
      if (mean_dT >= config_->trans_diff_thresh ||
          mean_dR >= config_->rot_diff_thresh) {
        CLOG(WARNING, "lidar.localization_geo_aware_icp") << "ICP did not converge to the specified threshold.";
        if (!refinement_stage) {
          CLOG(WARNING, "lidar.localization_geo_aware_icp") << "ICP did not enter refinement stage at all.";
        }
      }
      break;
    }
  ...
  /// Dump timing info
  CLOG(DEBUG, "lidar.localization_geo_aware_icp") << "Dump timing info inside loop: ";
  for (size_t i = 0; i < clock_str.size(); i++) {
    CLOG(DEBUG, "lidar.localization_geo_aware_icp") << clock_str[i] << timer[i]->count();
  }
  ...
  if (matched_points_ratio > config_->min_matched_ratio) {
    ...
    *qdata.loc_success = true;
    ...
    CLOG(WARNING, "lidar.localization_geo_aware_icp")
