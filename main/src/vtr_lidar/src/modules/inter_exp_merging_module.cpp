...
void InterExpMergingModule::configFromROS(const rclcpp::Node::SharedPtr &node,
  ...
  // clang-format off
  config_->dynamic_detection = node->declare_parameter<std::string>(param_prefix + ".dynamic_detection", config_->dynamic_detection);
...
void InterExpMergingModule::runAsyncImpl(QueryCache &qdata0,
  ...
  /// Check if this vertex has dynamic removed map
  {
    ...
    // Check if there's dependency not met
      ...
      auto dep_module = getFactory()->get(config_->dynamic_detection);
