...
/**
 * \file ouster_canoe_conversion_module.cpp
 * \author Autonomous Space Robotics Lab (ASRL)
 */
#include "vtr_lidar/modules/preprocessing/conversions/ouster_canoe_conversion_module.hpp"
...
void OusterCANOECart2Pol(pcl::PointCloud<PointWithInfo> &point_cloud) {
...
auto OusterCANOEConversionModule::Config::fromROS(
...
void OusterCANOEConversionModule::run_(QueryCache &qdata0, OutputCache &,
                                    const Graph::Ptr &,
  ...
  for (size_t idx = 0; idx < (size_t)points.rows(); idx++) {
    ...
    // pointwise timestamp
    point_cloud->at(idx).timestamp = static_cast<int64_t>(points(idx, 4) * 1e9);
  }

  // Ouster CANOE has no polar coordinates, so compute them manually.
  OusterCANOECart2Pol(*point_cloud);
  ...
  if (config_->visualize) {
    ...
    std::for_each(point_cloud_tmp.begin(), point_cloud_tmp.end(),
                  [&](PointWithInfo &point) {
                    point.flex21 =
                        static_cast<float>(point.timestamp) / 1e9;
                  });
