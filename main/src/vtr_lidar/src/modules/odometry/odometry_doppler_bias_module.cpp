...
/**
 * \file odometry_doppler_bias_module.cpp
 * \author Autonomous Space Robotics Lab (ASRL)
 */
#include "vtr_lidar/modules/odometry/odometry_doppler_bias_module.hpp"
...
auto OdometryDopplerBiasModule::Config::fromROS(const rclcpp::Node::SharedPtr &node,
  ...
  if (qcd.size() != 6) {
    ...
    CLOG(ERROR, "lidar.odometry_doppler_bias") << err;
...
OdometryDopplerBiasModule::OdometryDopplerBiasModule(const Config::ConstPtr &config,
...
void OdometryDopplerBiasModule::run_(QueryCache &qdata0, OutputCache &,
  ...
  if (!qdata.doppler_preprocessed_point_cloud ||
      !qdata.gyro_msgs) {
    CLOG_IF(!qdata.doppler_preprocessed_point_cloud, WARNING, "lidar.odometry_doppler_bias") << "Doppler preprocessed point cloud not initialized.";
    CLOG_IF(!qdata.gyro_msgs, WARNING, "lidar.odometry_doppler_bias") << "Gyro messages not initialized.";
  ...
  if(query_stamp < timestamp_prior) { 
    CLOG(WARNING, "lidar.odometry_doppler_bias") << "Query stamp: " << query_stamp << " is earlier than prior stamp: " << timestamp_prior;
    CLOG(WARNING, "lidar.odometry_doppler_bias") << "Difference between the two stamps is " << (query_stamp - timestamp_prior) << " ns";
  ...
  CLOG(DEBUG, "lidar.odometry_doppler_bias")
  ...
  if (prev_time == query_time && frame_count > 0) {
    CLOG(WARNING, "lidar.odometry_doppler_bias") << "Skipping point cloud with duplicate stamp";
    *qdata.odo_success = false;
    return;
  }
  ...
  CLOG(DEBUG, "lidar.odometry_doppler_bias") << "Max inliers: " << max_inliers;

  pcl::PointCloud<PointWithInfo> points;
  if (max_inliers == 0) {
    CLOG(WARNING, "lidar.odometry_doppler_bias") << "No inliers found in RANSAC.";
  ...
  CLOG(DEBUG, "lidar.odometry_doppler_bias") << "********Previous velocity estimate: " << prev_w_m_r_in_r_odo.transpose();
  CLOG(DEBUG, "lidar.odometry_doppler_bias") << "********Current velocity estimate: " << w_m_r_in_r_eval_.transpose();

  CLOG(DEBUG, "lidar.odometry_doppler_bias") << "Current velocity difference: " << vel_diff.transpose();
  CLOG(DEBUG, "lidar.odometry_doppler_bias") << "Current translational velocity difference: " << trans_vel_diff_norm;
  CLOG(DEBUG, "lidar.odometry_doppler_bias") << "Current rotational velocity difference: " << rot_vel_diff_norm;

  if (trans_vel_diff_norm > config_->max_trans_vel_diff || rot_vel_diff_norm > config_->max_rot_vel_diff) {
    CLOG(WARNING, "lidar.odometry_doppler_bias") << "Velocity difference between initial and final is too large: " << vel_diff_norm << " translational velocity difference: " << trans_vel_diff_norm << " rotational velocity difference: " << rot_vel_diff_norm;
  ...
  CLOG(DEBUG, "lidar.odometry_doppler_bias") << "T_r_m_eval: " << T_temp.matrix();
  CLOG(DEBUG, "lidar.odometry_doppler_bias") << "w_m_r_in_r_eval: " << w_temp.transpose();
  ...
  if (frame_count > 0 &&
    ...
    CLOG(DEBUG, "lidar.odometry_doppler_bias") << "using VTR preprocessed point cloud";
...
#pragma omp parallel for schedule(dynamic, 10) num_threads(config_->num_threads)
    ...
    CLOG(DEBUG, "lidar.odometry_doppler_bias") << "aligned_points size: " << aligned_points.size();
    ...
    *qdata.odo_success = true;
    ...
    CLOG(DEBUG, "lidar.odometry_doppler_bias") << "T_r_v_odo: " << *qdata.T_r_v_odo;
    CLOG(DEBUG, "lidar.odometry_doppler_bias") << "w_v_r_in_r_odo: " << *qdata.w_v_r_in_r_odo;
    CLOG(DEBUG, "lidar.odometry_doppler_bias") << "T_r_m_odo: " << *qdata.T_r_m_odo;
  } 

  if (frame_count > 0 && !estimate_reasonable) {
    CLOG(WARNING, "lidar.odometry_doppler_bias") << "Doppler Odometry failed";
    CLOG(DEBUG, "lidar.odometry_doppler_bias") << "using VTR preprocessed point cloud";
    ...
    cart2pol(*undistorted_point_cloud);  // correct polar coordinates.
    qdata.undistorted_point_cloud = undistorted_point_cloud;
    CLOG(DEBUG, "lidar.odometry_doppler_bias") << "undistorted_point_cloud size: " << undistorted_point_cloud->size();
    *qdata.odo_success = false;  
  }
  timer[3]->stop();
  ...
  /// Dump timing info
  CLOG(DEBUG, "lidar.odometry_doppler_bias") << "Dump timing info inside loop: ";
  for (size_t i = 0; i < clock_str.size(); i++) {
    CLOG(DEBUG, "lidar.odometry_doppler_bias") << "  " << clock_str[i] << timer[i]->count() / 1e6;
  }
  // clang-format on
}
