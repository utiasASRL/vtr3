...
CasadiUnicycleFollowerMPC::CasadiUnicycleFollowerMPC( bool verbose, casadi::Dict ipopt_opts){
  casadi::Dict opts;
  if (!verbose) { 
    opts["print_time"] = 0;
    ipopt_opts["print_level"] = 0;
...
std::map<std::string, casadi::DM> CasadiUnicycleFollowerMPC::solve(const Config& mpcConf) {
  using namespace casadi;
  std::map<std::string, DM> arg;
