...
TEST_P(CasadiTests, UnicycleMPC) {

  const auto& [init_pose_l, _] = GetParam();
...
TEST_P(CasadiTests, UnicycleFollowerMPC) {

  const auto& [init_pose_l, init_pose_f] = GetParam();
  

  CasadiUnicycleMPCFollower solver;
  CasadiUnicycleMPCFollower::Config config;
  config.VF = 1.0;
  config.vel_max = {1.5, 0.5};
  config.vel_min = {0, -0.5};
  config.Q_x = 10;
  ...
  config.previous_vel = {0.0, 0.0};
  config.T0 = {std::get<0>(init_pose_f), std::get<1>(init_pose_f), std::get<2>(init_pose_f)};

  for(int i = 1; i < config.N+1; i++){
    config.reference_poses.push_back({std::get<0>(init_pose_f) + i*config.VF*config.DT, 0, 0});
  }

  for(int i = 1; i < config.N+1; i++){
    config.leader_reference_poses.push_back({std::get<0>(init_pose_l) + i*config.VF*config.DT, 0, 0});
  }
...
TEST_P(CasadiTests, BicycleMPC) {

  const auto& [init_pose_l, _] = GetParam();
  

  CasadiBicycleMPC solver;
  CasadiBicycleMPC::Config config;
  config.VF = 1.0;
  config.vel_max = {1.5, 0.5};
  config.vel_min = {0, -0.5};
  config.lin_acc_max = 5.0;
  config.ang_acc_max = 5.0;
  config.Q_lat = 10;
  config.Q_lon = 4.0;
  config.Q_th = 7;
  config.R1 = 0;
  ...
  for(int i = 1; i < config.N+1; i++){
    config.reference_poses.push_back({std::get<0>(init_pose_l) + i*config.VF*config.DT, 0, 0});
    config.cost_weights.push_back(1.0);
  }
...
TEST_P(CasadiTests, BicycleFollowerMPC) {

  const auto& [init_pose_l, init_pose_f] = GetParam();
  

  CasadiBicycleMPCFollower solver;
  CasadiBicycleMPCFollower::Config config;
  config.VF = 1.0;
  config.vel_max = {1.5, 0.5};
  config.vel_min = {0, -0.5};
  config.lin_acc_max = 5.0;
