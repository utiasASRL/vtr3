...
from acker_cycle_solver import solver, motion_model, alpha, N, step_horizon, n_states, n_controls
...
lbg = ca.DM.zeros((n_states*(N+1) + 3*N, 1))  # constraints lower bound
ubg = ca.DM.zeros((n_states*(N+1) + 3*N, 1))  # constraints upper bound

#Corridor Width constraints
lbg[n_states*(N+1):n_states*(N+1)+N] = -ca.inf
ubg[n_states*(N+1):n_states*(N+1)+N] = ca.inf

lbg[n_states*(N+1)+N:n_states*(N+1)+3*N:2] = 0
ubg[n_states*(N+1)+N:n_states*(N+1)+3*N:2] = ca.inf

lbg[n_states*(N+1)+N+1:n_states*(N+1)+3*N:2] = -ca.inf
ubg[n_states*(N+1)+N+1:n_states*(N+1)+3*N:2] = 0
