...
class IncrementalBOWTrainer
    : public cv::BOWTrainer {

public:
  /// Construct the trainer with config
  IncrementalBOWTrainer(
      double cluster_size = 0.4,  ///< The minimum distance between words in the vocabulary
      bool first_not_mean = true  ///< Add distant features to the vocabulary immediately, don't find the mean of a new cluster
      );
  ...
  /// Note: This assumes you only pass it descriptors that didn't match against the existing vocabulary,
  /// that check is up to you!
  virtual cv::Mat cluster() const;  
  /// Returns trained vocabulary (cluster centers)
  /// This function is non-persistent, and doesn't save anything to members
  virtual cv::Mat cluster(
      const cv::Mat& descriptors  ///< The descriptors we'd like to cluster into a vocabulary
      ) const;

  /// Returns indices to the vocabulary, and optionally the closest cluster center for each descriptor.
  /// This is the main clustering function
  std::vector<unsigned> clusterFirstByIndex(
      const cv::Mat & descriptors,              ///< The descriptors to cluster into a vocabulary
      std::vector<unsigned> * closest = nullptr ///< [out] optionally, the closest vocabulary word
      ) const;
...
protected:
  ...
  /// Whether to use the first descriptor (fast) or the mean (accurate) as the cluster center
