...
auto GyroBiasEstimationModule::Config::fromROS(
  ...
  config->alpha = 1 / double(window_size);
  ...
  try {
    ...
    if (last_vel.head<3>().norm() < config_->max_vel) {
      for(const auto& gyro_msg : *qdata.gyro_msgs) {
        gyro_bias_ = (1 - config_->alpha) * gyro_bias_ + config_->alpha * Eigen::Vector3d(gyro_msg.angular_velocity.x, gyro_msg.angular_velocity.y, gyro_msg.angular_velocity.z);
      }

      CLOG_EVERY_N(10, DEBUG, static_name) << "Gyro bias is now: " << gyro_bias_;
    }

    for(auto& gyro_msg : *qdata.gyro_msgs) {
      gyro_msg.angular_velocity.x -= gyro_bias_(0);
      gyro_msg.angular_velocity.y -= gyro_bias_(1);
      gyro_msg.angular_velocity.z -= gyro_bias_(2);
    }
