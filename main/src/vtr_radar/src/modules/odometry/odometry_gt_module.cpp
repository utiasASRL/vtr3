...
void OdometryGTModule::run_(QueryCache &qdata0, OutputCache &,
  ...
  CLOG(DEBUG, "radar.odometry_gt") << "Start building a kd-tree of the map.";
...
#pragma omp parallel for schedule(dynamic, 10) num_threads(config_->num_threads)
  ...
  CLOG_IF(qdata.gyro_msgs, DEBUG, "radar.odometry_gt") << "Gyro messages are available.";
  CLOG_IF(qdata.doppler_scan, DEBUG, "radar.odometry_gt") << "Doppler scan is available.";
  CLOG_IF(config_->remove_orientation, DEBUG, "radar.odometry_gt") << "Removing ICP orientation contribution.";
  for (int step = 0;; step++) {
    /// sample points
    timer[0]->start();
...
#pragma omp critical(odo_icp_add_doppler_error_cost)
{
    ...
    if (config_->use_vel_meas) {
      ...
      if (vel_meas[0] != -1000.0) {
        ...
        CLOG(ERROR, "radar.odometry_gt") << "Velocity measurement not available.";
      }
    }
...
#pragma omp parallel for schedule(dynamic, 10) num_threads(config_->num_threads)
  ...
  /// Dump timing info
  CLOG(DEBUG, "radar.odometry_gt") << "Dump timing info inside loop: ";
  for (size_t i = 0; i < clock_str.size(); i++) {
    CLOG(DEBUG, "radar.odometry_gt") << "  " << clock_str[i] << timer[i]->count();
  }
  ...
  if (trans_vel_diff_norm > config_->max_trans_vel_diff || rot_vel_diff_norm > config_->max_rot_vel_diff) {
    CLOG(WARNING, "radar.odometry_gt") << "Velocity difference between initial and final is too large: " << vel_diff_norm << " translational velocity difference: " << trans_vel_diff_norm << " rotational velocity difference: " << rot_vel_diff_norm;
    estimate_reasonable = false;
  }

  if (diff_T_trans > config_->max_trans_diff) {
    CLOG(WARNING, "radar.odometry_gt") << "Transformation difference between initial and final translation is too large. Transform difference vector: " << diff_T.transpose();
    estimate_reasonable = false;
  }
  if (diff_T_rot > config_->max_rot_diff) {
    CLOG(WARNING, "radar.odometry_gt") << "Transformation difference between initial and final rotation is too large. Transform difference vector: " << diff_T.transpose();
    estimate_reasonable = false;
  }
...
#pragma omp parallel for schedule(dynamic, 10) num_threads(config_->num_threads)
    ...
    *qdata.odo_success = true;
    ...
    if (matched_points_ratio <= config_->min_matched_ratio) {
      CLOG(WARNING, "radar.odometry_gt")
