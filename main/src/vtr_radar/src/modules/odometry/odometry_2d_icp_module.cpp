...
/**
 * \file odometry_2d_icp_module.cpp
 * \author Daniil Lisus, Yuchen Wu, Keenan Burnett, Autonomous Space Robotics Lab (ASRL)
 */
#include "vtr_radar/modules/odometry/odometry_2d_icp_module.hpp"
...
auto Odometry2dICPModule::Config::fromROS(const rclcpp::Node::SharedPtr &node,
  ...
  if (qcd.size() != 3) {
    ...
    CLOG(ERROR, "radar.odometry_2d_icp") << err;
...
void Odometry2dICPModule::run_(QueryCache &qdata0, OutputCache &,
  ...
  if (!qdata.sliding_map_odo) {
    ...
    CLOG(INFO, "radar.odometry_2d_icp") << "First frame, simply return.";
    // clang-format off
  ...
  CLOG(DEBUG, "radar.odometry_2d_icp")
  ...
  CLOG(DEBUG, "radar.odometry_2d_icp") << "Timestamps of concern frame stamp: " << (scan_stamp - timestamp_prior) /1e9 << " timestamp_end " << (frame_end_time - timestamp_prior) /1e9;
  ...
  {
    CLOG(DEBUG, "radar.odometry_2d_icp") << "Last odometry and gyro preintegration update is more recent than radar scan.";
  ...
  // Set up priors
  CLOG(DEBUG, "radar.odometry_2d_icp") << "Adding prior to trajectory.";
  ...
  CLOG(DEBUG, "radar.odometry_2d_icp") << "Start building a kd-tree of the map.";
  ...
  CLOG(DEBUG, "radar.odometry_2d_icp") << "Start initial alignment.";
...
#pragma omp parallel for schedule(dynamic, 10) num_threads(config_->num_threads)
  ...
  CLOG(DEBUG, "radar.odometry_2d_icp") << "Start the ICP optimization loop.";
  CLOG_IF(qdata.gyro_msgs, DEBUG, "radar.odometry_2d_icp") << "Gyro messages are available.";
  CLOG_IF(qdata.doppler_scan, DEBUG, "radar.odometry_2d_icp") << "Doppler scan is available.";
  CLOG_IF(config_->remove_orientation, DEBUG, "radar.odometry_2d_icp") << "Removing ICP orientation contribution.";
  for (int step = 0;; step++) {
    /// sample points
    timer[0]->start();
...
//     }
    ...
    //   } else {
    //     CLOG(ERROR, "radar.odometry_2d_icp") << "Velocity measurement not available.";
    //   }
    // }
    ...
    try {
      ...
      CLOG(WARNING, "radar.odometry_2d_icp") << "STEAM failed to solve, skipping frame. Error message: " << e.what();
      solver_failed = true;
    }
...
#pragma omp parallel for schedule(dynamic, 10) num_threads(config_->num_threads)
    ...
    if (!refinement_stage && step >= first_steps) {
      if ((step >= max_it - 1) || (mean_dT < config_->trans_diff_thresh &&
        ...
        CLOG(DEBUG, "radar.odometry_2d_icp") << "Initial alignment takes " << step << " steps.";
    ...
    if ((refinement_stage && step >= max_it - 1) ||
      ...
      //
      CLOG(DEBUG, "radar.odometry_2d_icp") << "Total number of steps: " << step << ", with matched ratio " << matched_points_ratio;
      if (mean_dT >= config_->trans_diff_thresh ||
          mean_dR >= config_->rot_diff_thresh) {
        CLOG(WARNING, "radar.odometry_2d_icp") << "ICP did not converge to the specified threshold";
        if (!refinement_stage) {
          CLOG(WARNING, "radar.odometry_2d_icp") << "ICP did not enter refinement stage at all.";
        }
      }
      break;
    }
  ...
  /// Dump timing info
  CLOG(DEBUG, "radar.odometry_2d_icp") << "Dump timing info inside loop: ";
  for (size_t i = 0; i < clock_str.size(); i++) {
    CLOG(DEBUG, "radar.odometry_2d_icp") << "  " << clock_str[i] << timer[i]->count();
  }
  ...
  CLOG(DEBUG, "radar.odometry_2d_icp") << "Current transformation difference: " << diff_T.transpose();
  CLOG(DEBUG, "radar.odometry_2d_icp") << "Current velocity difference: " << vel_diff.transpose();

  if (trans_vel_diff_norm > config_->max_trans_vel_diff || rot_vel_diff_norm > config_->max_rot_vel_diff) {
    CLOG(WARNING, "radar.odometry_2d_icp") << "Velocity difference between initial and final is too large: " << vel_diff_norm << " translational velocity difference: " << trans_vel_diff_norm << " rotational velocity difference: " << rot_vel_diff_norm;
    estimate_reasonable = false;
  }

  if (diff_T_trans > config_->max_trans_diff) {
    CLOG(WARNING, "radar.odometry_2d_icp") << "Transformation difference between initial and final translation is too large. Transform difference vector: " << diff_T.transpose();
    estimate_reasonable = false;
  }
  if (diff_T_rot > config_->max_rot_diff) {
    CLOG(WARNING, "radar.odometry_2d_icp") << "Transformation difference between initial and final rotation is too large. Transform difference vector: " << diff_T.transpose();
    estimate_reasonable = false;
  }
  ...
  if (matched_points_ratio > config_->min_matched_ratio && estimate_reasonable && !solver_failed) {
    ...
    *qdata.timestamp_odo = timestamp_odo_new;
    ...
    CLOG(DEBUG, "radar.odometry_2d_icp") << "T_m_r is: " << qdata.T_r_m_odo->inverse().vec().transpose();
    CLOG(DEBUG, "radar.odometry_2d_icp") << "w_m_r_in_r is: " << qdata.w_m_r_in_r_odo->transpose();
    //
    ...
    *qdata.timestamp_prior = frame_end_time;
    ...
    //
    *qdata.odo_success = true;
    CLOG(DEBUG, "radar.odometry_2d_icp") << "Odometry successful. T_r_m_icp: " << T_r_m_icp;
    CLOG(DEBUG, "radar.odometry_2d_icp") << "T_r_v_odo: " << *qdata.T_r_v_odo;
  } else {
    if (matched_points_ratio <= config_->min_matched_ratio) {
      CLOG(WARNING, "radar.odometry_2d_icp")
