...
bool load_gt_pose(std::ifstream &pose_stream, Eigen::Matrix<double, 6, 1> &v_gt, int64_t target_timestamp) {
  std::string line;
  while (std::getline(pose_stream, line)) {
      ...
      if (gt[0] == target_timestamp/1000) {  // timestamp match (use tolerance if needed)
          Eigen::Matrix4d T_ab = Eigen::Matrix4d::Identity();
          T_ab.block<3, 3>(0, 0) = rpy2rot(gt[7], gt[8], gt[9]); // RPY assumed in radians
          T_ab.block<3, 1>(0, 3) << gt[1], gt[2], gt[3];

          // Linear velocity in B
          Eigen::Vector4d v_a_tilde(gt[4], gt[5], gt[6], 1);
          Eigen::Vector4d v_b_tilde = T_ab.transpose() * v_a_tilde;
          v_gt.head<3>() = v_b_tilde.head<3>();
