...
void ICPModule::runImpl(QueryCache &qdata, MapCache &mdata,
  ...
  if (config_->source == "live" && !mdata.current_map) {
  ...
  auto &map =
      config_->source == "live" ? *mdata.current_map : *mdata.current_map_loc;
  // output
  auto &T_r_m = config_->source == "live" ? *mdata.T_r_m : *mdata.T_r_m_loc;
