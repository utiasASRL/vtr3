...
class BaseId {
 public:
   ...
   *
   * Many standard containers expect this function to exist
   */
  inline bool operator<(const BaseId &rhs) const {
...
#if 0
template <class ENUM>
EXTEND_BOOST_HASH(BaseTypedId<ENUM>)
#endif
}  // namespace pose_graph
}  // namespace vtr
...
class BasePairedId {
 public:
   ...
   * \brief Get the run id
   *
   * \todo (yuchen) This does not look corrent.
   */
...
#if 0
template <class ENUM>
EXTEND_BOOST_HASH(BasePairedId<ENUM>)
#endif
}  // namespace pose_graph
}  // namespace vtr
