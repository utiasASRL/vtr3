...
int main() {
  ...
  for (int vertex_idx = 0; vertex_idx < 10; ++vertex_idx) {
    ...
    std::cout << "Store image " << vertex_idx << " into vertex "
              << vertex_id << std::endl;
    /// robochunk::std_msgs::TimeStamp stamp;
    cv::Mat image = cv::imread((image_dir / (std::to_string(vertex_idx+1)+".jpg")).string(), cv::IMREAD_COLOR);
    ...
    vertex->insert<sensor_msgs::msg::Image>(stream_name, *img_msg, stamp);
  }
