...
void KeyframeOptimizationModule::addPosePrior(MapCache &mdata) {
  ...
  EdgeTransform &pose_prior = *mdata.T_q_m_prior;
...
bool KeyframeOptimizationModule::verifyInputData(QueryCache &,
                                                 MapCache &mdata) {
  // sanity check
  if ((mdata.success.is_valid() && *mdata.success == false) ||
      *mdata.map_status == MAP_NEW) {
    return false;
  }
  ...
  if (mdata.ransac_matches.is_valid() == false) {
    ...
    mdata.T_q_m = lgmath::se3::Transformation();
    return false;
  }
...
void KeyframeOptimizationModule::computeTrajectory(
  ...
  for (; itr != graph->end(); ++itr) {
    ...
    int64_t next_prev_dt = next_stamp.nanoseconds_since_epoch() -
                           prev_stamp.nanoseconds_since_epoch();
  ...
  int64_t query_map_dt = query_stamp.nanoseconds_since_epoch() -
                         map_stamp.nanoseconds_since_epoch();
  ...
  steam::Time map_time(
      static_cast<int64_t>(map_stamp.nanoseconds_since_epoch()));
...
void KeyframeOptimizationModule::updateCaches(QueryCache &qdata,
  ...
  mdata.T_q_m = query_pose_->getValue();
