...
namespace std {
  std::ostream &
  operator<<(std::ostream & os, const asrl::status_msgs::ExpRecogStatus & msg) {
    std::ios fmt(nullptr); fmt.copyfmt(os);
    os << std::fixed;
//    if(msg.has_algorithm()) os << msg.Algorithm_Name(msg.algorithm()) << " ";
    if(msg.has_in_the_loop()) os << (msg.in_the_loop() ? "[itl] " : "[off] ");
    if (msg.has_computation_time_ms()) os << "run: " << std::setw(5) << std::setprecision(1)
                                          << msg.computation_time_ms() << " ms ";
    if (msg.has_load_time_ms()) os << "load: " << std::setw(4) << std::setprecision(1)
                                   << msg.load_time_ms() << " ms ";
    ...
    if (msg.cosine_distances_size()) {
      ...
      std::ostringstream oss; oss << std::setprecision(3) << std::fixed;
...
RunIdSet
getRunIds(const pose_graph::RCGraphBase & graph) {
  RunIdSet rids;
...
RunIdSet
privelegedRuns(const pose_graph::RCGraphBase & graph, RunIdSet rids) {
  RunIdSet prids;
...
maskSubgraph(const pose_graph::RCGraphBase::Ptr & graph,
  ...
  for (VertexId vid : graph->subgraph().getNodeIds())
    if (mask.count(vid.majorId()))
      kept_vertex_ids.push_back(vid);
  return graph->getSubgraph(kept_vertex_ids);
}


RunIdSet
fillRecommends(RunIdSet * recommends, const ScoredRids & distance_rids, unsigned n) {
  RunIdSet new_recs;
...
void ExperienceTriageModule::run(
  QueryCache & qdata,
  MapCache & mdata,
  ...
  if (config_.in_the_loop) {
    ...
    if (recommended.empty()) {
      ...
      if (mdata.localization_status)
        mdata.localization_status->set_window_num_vertices(submap_ptr->numberOfVertices());
    }
  }
...
void ExperienceTriageModule::updateGraph(
  QueryCache & qdata,
  MapCache & mdata,
