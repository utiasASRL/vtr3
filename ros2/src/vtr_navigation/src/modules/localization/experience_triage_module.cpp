...
namespace std {
std::ostream &
operator<<(std::ostream &os, const vtr_messages::msg::ExpRecogStatus &msg) {
  std::ios fmt(nullptr);
  fmt.copyfmt(os);
  os << std::fixed;
  ...
  os << "load: " << std::setw(4) << std::setprecision(1)
     << msg.load_time_ms << " ms ";
  ...
  if (config_.in_the_loop) {
    ...
    if (recommended.empty()) {
      ...
      if (mdata.localization_status)
        mdata.localization_status->set__window_num_vertices(submap_ptr->numberOfVertices());
    }
  }
