...
// vvv MAIN AND INITIALIZATION vvv //
...
void MelRecognitionModule::run(QueryCache & qdata, MapCache & mdata,
  ...
  RigLandmarkConstPtrs live_rig_lm_ptrs; live_rig_lm_ptrs.reserve(live_suite_lms.size());
  ...
  // TODO maybe this piece can move to the localization pipeline
  {
...
MelRecognitionModule::getMapVertices(const Graph::ConstPtr & graph,
  ...
  // status
  if (config_->verbose) {
    std::stringstream neighbour_ss; neighbour_ss << "Adding map: ";
...
int MelRecognitionModule::matchVertex(const RigLandmarkConstPtrs & query_suite_lm,
  ...
  for (uint32_t rig_id = 0; rig_id < query_suite_lm.size(); ++rig_id) {
    ...
    auto rig_vocab_msg
        = map_vertex->retrieveKeyframeData<vision_msgs::RigFeatures>(stream_name);
    ...
    if ((unsigned)rig_vocab_msg->channels_size() != query_rig_lm.size()) {
      LOG(WARNING) << "live has " << query_rig_lm.size()
                << " channels, but vocab has " << rig_vocab_msg->channels_size();
    }
    unsigned max_num_channels = std::min((unsigned)rig_vocab_msg->channels_size(),
    ...
    for (uint32_t channel_id = 0; channel_id < max_num_channels; ++channel_id) {
      ...
      const auto & channel_vocab_msg = rig_vocab_msg->channels().Get(channel_id);
      ...
      if (channel_vocab_msg.cameras_size() == 0) {
      ...
      total_matched += matchChannel(query_channel_lm, query_lm_id, construct_ptr,
                                    channel_vocab_msg.cameras().Get(0),
                                    vocab_lm_id);
    }
  }
...
int
MelRecognitionModule::matchChannel(
    const vision::Features & query_channel_lm,
    vision::LandmarkId query_lm_id,
    BowConstruct * construct_ptr,
    const vision_msgs::Features & map_channel_vocab_msg,
  ...
  unsigned total_matched = 0;
  for (int32_t map_vocab_id = 0; map_vocab_id < map_channel_vocab_msg.keypoint_info_size(); ++map_vocab_id) {
    map_vocab_lm_id.index = map_vocab_id;
    // map landmark refs
    const auto & map_keypoint_info = map_channel_vocab_msg.keypoint_info().Get(map_vocab_id);
    const auto & map_descriptor_string = map_channel_vocab_msg.descriptors();
    float *map_descriptor = (float*)&map_descriptor_string.data()[step_size*map_vocab_id];
    ...
    for(uint32_t query_id = 0; query_id < query_channel_lm.feat_infos.size(); ++query_id) {
      ...
      if(channel_matched[query_id].tracked == false ||
         channel_matched[query_id].worded == true) continue;
      ...
      const float * query_descriptor = query_channel_lm.descriptors.ptr<float>(query_id);
...
MelRecognitionModule::clusterUnmatchedObs(
  ...
  for (uint32_t rig_id = 0; rig_id < query_suite_lm.size(); ++rig_id) {
    ...
    for (uint32_t channel_id = 0; channel_id < rig_landmarks.channels.size(); ++channel_id) {
      ...
      std::vector<unsigned> vocab_mask; vocab_mask.reserve(center_uids.size());
...
vision::BowDescriptor recallBow(const Vertex::Ptr & v) {
  ...
  const auto & bow_msg
      = v->retrieveKeyframeData<vision_msgs::BowDescriptor>("/bagofwords");
...
void MelRecognitionModule::recordResults(
  ...
  // The keyframe time
  *status_msg.mutable_keyframe_time() = query_vertex->keyFrameTime();
  // The query id
  status_msg.set_query_id(query_vertex->id());
  // The algorithm
  status_msg.set_algorithm(status_msgs::ExpRecogStatus_Algorithm_BAGOFWORDS);
  // Whether we're running online
  status_msg.set_in_the_loop(config_->in_the_loop);
  ...
  for (const ExperienceDifference & difference : differences) {
    ...
    dist_msg.set_run_id(difference.first);
    dist_msg.set_cosine_distance(difference.second);
  }
...
void MelRecognitionModule::updateGraph(
  ...
  for (const vision::RigFeatures & rig_vocabulary2 : vocabulary2) {
    ...
    vision_msgs::RigFeatures rc_rig_vocabulary2
        = messages::copyFeatures(rig_vocabulary2);

    // Save to the graph
    std::string vocab_stream = "/" + rig_name + "/vocabulary";
    if (!graph->hasVertexStream(rid, vocab_stream)) {
      graph->registerVertexStream(rid, vocab_stream, true);
    }
  ...
  if (status_msg_to_save_.query_id() == vid) {
    ...
    std::string results_stream = "/results/experience_picker";
