...
uint32_t LocalizationSearchPipeline::_getNextTrunkSeq() {
  // Reset the search width if we have localized in the last 5 attempts (we must
  // be close)
  if (locSuccesses_ > 0) {
  ...
  auto trunkSeq = tactic->getLocalizationChain().trunkSequenceId();
  auto trunkSpan = std::max(
      trunkSeq,
      uint32_t(tactic->getLocalizationChain().sequence().size()) - trunkSeq);
  ...
  // If the offset is too big, reset to zero without incrementing the search
  // window
  ...
  // If we move past the window, increment the window width and restart from
  // zero offset
  ...
  if (newSeq < 0 ||
      newSeq >= int(tactic->getLocalizationChain().sequence().size())) {
    // Recursion: skip this sequence because it was past an endpoint.  Return
    // the next sequence, as we haven't exceeded both endpoints yet (checked
    // above)
...
void LocalizationSearchPipeline::_updateRunLoc(QueryCachePtr q_data,
                                               MapCachePtr m_data) {
...
void LocalizationSearchPipeline::_updateTrunkLoc() {
  tactic->updatePersistentLocalization(
      tactic->getLocalizationChain().trunkVertexId(),
      tactic->getLocalizationChain().T_leaf_trunk());
  LOG(DEBUG)
      << "Localization search successfully updated persistent localization at "
      << tactic->getLocalizationChain().trunkVertexId() << ".";
  const lgmath::se3::TransformationWithCovariance &T_lt =
      tactic->getLocalizationChain().T_leaf_trunk();
