...
void showFeatures(std::mutex &vis_mtx, QueryCache &qdata, std::string suffix) {
  // check if the required data is in the cache
  if (!qdata.rig_images.is_valid() || !qdata.rig_features.is_valid() ||
      !qdata.candidate_landmarks.is_valid()) {
    return;
...
void showMatches(std::mutex &vis_mtx, QueryCache &qdata, MapCache &mdata,
  ...
  for (; features_itr != features.end() && matches_itr != matches.end() &&
    ...
    if (mdata.T_q_m_prior.is_valid() == true) {
      lgmath::se3::Transformation T_q_m =
          (*qdata.T_sensor_vehicle) * (*mdata.T_q_m_prior) *
...
void showMelMatches(std::mutex &vis_mtx, QueryCache &qdata, MapCache &mdata,
  ...
  auto input_image = messages::wrapImage(*ros_image.get());
