...
//          << "\n" );
  ...
  EXPECT_NE(tf * pts_ref.rightCols<n_outliers>(), pts_cam.rightCols<n_outliers>());
  ...
  // Set up RANSAC
  auto verifier = std::make_shared<VerifySampleIndices>(); // Need 1 close feature
...
//          << "\n" );
  ...
  auto verifier = std::make_shared<VerifySampleIndices>(); // Need 1 close feature
  auto sampler = std::make_shared<BasicSampler>(verifier);
  VanillaRansac<StereoTransformModel::SolutionType> ransac(sampler, 3.5, 5.0, 100, 0.25, 150);
  ransac.setCallback(model);
