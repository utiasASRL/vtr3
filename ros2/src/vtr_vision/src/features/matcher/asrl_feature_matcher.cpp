...
SimpleMatches ASRLFeatureMatcher::matchStereoFeatures(const Features &frame1, const Features &frame2){
  Eigen::Matrix3d H = Eigen::Matrix3d::Identity();
  return matchFeatures(frame1, frame2, H,
                       config_.stereo_x_tolerance_min_,
                       config_.stereo_x_tolerance_max_,
                       config_.stereo_y_tolerance_);
}

SimpleMatches ASRLFeatureMatcher::matchFeatures(const Features &frame1, const Features &frame2, const float &window_size) {
  Eigen::Matrix3d H = Eigen::Matrix3d::Identity();
  return matchFeatures(frame1, frame2, H, -window_size, window_size, window_size);
}
...
SimpleMatches ASRLFeatureMatcher::matchFeatures(const Features &frame1,
  ...
  if(frame1.feat_type.impl != frame2.feat_type.impl) {
    LOG(ERROR) << "ASRLFeatureMatcher::matchfeatures(): feature types do not match!";
    return matches;
  }
...
#pragma omp parallel for num_threads(config_.num_threads_)
  ...
  for(unsigned ii = 0; ii < frame1.keypoints.size(); ii++) {
    ...
    for(unsigned jj = 0; jj < frame2.keypoints.size(); jj++) {
      ...
      if(checkConditions(kp1, frame1.feat_infos[ii], frame2.keypoints[jj], frame2.feat_infos[jj],
                         eh, x_window_min, x_window_max_mod, y_window_size, type)) {
        ...
        if(frame1.feat_type.impl == FeatureImpl::OPENCV_ORB) {
          dist = briefmatch(&frame1.descriptors.at<unsigned char>(ii,0), &frame2.descriptors.at<unsigned char>(jj,0), frame1.feat_type.bytes_per_desc);
        } else if(frame1.feat_type.impl == FeatureImpl::ASRL_GPU_SURF) {
          dist = surfmatch(&frame1.descriptors.at<float>(ii,0),&frame2.descriptors.at<float>(jj,0), frame1.feat_type.bytes_per_desc/sizeof(float));
        } else {
          LOG(ERROR) << "ASRLFeatureMatcher::matchfeatures(): unknown feature type!";
        }
...
bool ASRLFeatureMatcher::checkConditions(const Keypoint &kp1,
  ...
  if (type == CheckType::EPIPOLE ) {
    return checkEpipole(kp1, fi1, kp2,fi2,eh,x_window_size_min,x_window_size_max,y_window_size);
  } else {
    // since we don't have to or have already transformed the point, just do the normal check
    float x_dist = kp1.pt.x - kp2.pt.x;
    float y_dist = kp1.pt.y - kp2.pt.y;
    // do the two points lie close to each other within conditions?
    if(x_dist < x_window_size_min || x_dist > x_window_size_max || std::fabs(y_dist) > y_window_size) {
      return false;
    }
  }
...
bool ASRLFeatureMatcher::checkEpipole(const Keypoint &kp1,
                                      const FeatureInfo &fi1,
                                      const Keypoint &kp2,
                                      const FeatureInfo &fi2,
                                      const EpipoleHelper &eh,
  ...
  Point lp1;
  lp1.x = (eh.l(1)*(eh.l(1)*kp1.pt.x-eh.l(0)*kp1.pt.y) - eh.l(0)*eh.l(2))/eh.a2b2;
  lp1.y = (eh.l(0)*(-eh.l(1)*kp1.pt.x+eh.l(0)*kp1.pt.y) - eh.l(1)*eh.l(2))/eh.a2b2;

  // find the point on the line closest to kp2.pt;
  Point lp2;
  lp2.x = (eh.l(1)*(eh.l(1)*kp2.pt.x-eh.l(0)*kp2.pt.y) - eh.l(0)*eh.l(2))/eh.a2b2;
  lp2.y = (eh.l(0)*(-eh.l(1)*kp2.pt.x+eh.l(0)*kp2.pt.y) - eh.l(1)*eh.l(2))/eh.a2b2;

  // subtract lp2 from lp1 to put lp2 at the origin of the co-ordinate system we are about to rotate
  lp1.x -= lp2.x;
  lp1.y -= lp2.y;
  ...
  float y_dist = std::fabs(eh.l(0)*kp2.pt.x + eh.l(1)*kp2.pt.y+eh.l(2))/eh.sqrta2b2;

  if(x_dist < x_window_size_min || x_dist > x_window_size_max || y_dist > y_window_size) {
    return false;
  }
...
float ASRLFeatureMatcher::briefmatch(const unsigned char *d1, const unsigned char *d2, unsigned size) {
  ...
  //make the score compatible with our standard by subtracting from 1.0. 1.0 is worst match, 0.0 is perfect
...
float ASRLFeatureMatcher::surfmatch(const float *d1, const float *d2, unsigned size) {
...
/////////////////////////////////////////////////////////////////////////
void ASRLFeatureMatcher::addMatchingTask(const Features &frame1, const Features &frame2) {
  // spawn an asynchronous descriptor extraction call and put the future frame at the end of the queue.
  future_matches.emplace(std::async(std::launch::async,(SimpleMatches(ASRLFeatureMatcher::*)(const Features&, const Features&))&ASRLFeatureMatcher::matchFeatures,this,frame1,frame2));
}

/////////////////////////////////////////////////////////////////////////
// ASRLFeatureMatcher::addMatchingTask
/////////////////////////////////////////////////////////////////////////
void ASRLFeatureMatcher::addStereoMatchingTask(const Features &frame1, const Features &frame2) {
  // spawn an asynchronous descriptor extraction call and put the future frame at the end of the queue.
  future_stereomatches.emplace(std::async(std::launch::async,(SimpleMatches(ASRLFeatureMatcher::*)(const Features&, const Features&))&ASRLFeatureMatcher::matchStereoFeatures,this,frame1,frame2));
}
