...
asrl::safetyMonitor::path_tracker_monitor_input::path_tracker_monitor_input(ros::NodeHandle nh) :
  ...
  ********************/
  statusSubscriber_ = nodeHandle_.subscribe<asrl__control__path_tracker::StatusOut>("in/path_tracker_status",
                                                                                    100,
                                                                                    &path_tracker_monitor_input::statusCallback,
                                                                                    this);
  ...
  if (status->state == std::string("FOLLOWING") || status->state == std::string("PAUSED")) {
    ...
    if (acceptable_lateral_error == true && acceptable_heading_error == true) {
      ...
      double new_speed_limit =
          get_speed_increase(signal_monitors[pt_error_monitor].get_max_allowed_speed(), max_speed_filter_const_);
    ...
    if (flg_acceptable_pred_lateral_error == true && flg_acceptable_pred_heading_error == true) {
      ...
      double new_speed_limit =
          get_speed_increase(signal_monitors[pred_pt_error_monitor].get_max_allowed_speed(), max_speed_filter_const_);
    ...
    if (acceptable_loc_delay_time == false) {
      ...
      double new_speed_limit =
          get_speed_increase(signal_monitors[loc_delay_monitor].get_max_allowed_speed(), max_speed_filter_const_);
      signal_monitors[loc_delay_monitor].set_max_allowed_speed(new_speed_limit);
    }
