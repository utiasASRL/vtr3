...
void SequentialAppendWriter::open(
  ...
  if (converter_options.output_serialization_format !=
    ...
    converter_ = std::make_unique<rosbag2_cpp::Converter>(converter_options,
                                                          converter_factory_);
  }
  ...
  storage_ = storage_factory_->open_read_write(storage_uri,
  ...
  if (append_mode_) {
    ...
    std::cout << "Message count before appending: " << metadata_.message_count
              << std::endl;
    ...
    std::cout << "Number of topics before appending: "
