...
void MpcNominalModel::compute_velocities_from_state(Eigen::VectorXf &velocity,
...
void MpcNominalModel::extract_disturbance_dependencies(const MpcNominalModel::model_state_t &x_input,
...
bool MpcNominalModel::computeDisturbancesForExperienceKm2(MpcNominalModel::experience_t &experience_km2,
  ...
  bool state_size_correct = ((MODEL_INCLUDES_VELOCITY == true) && (STATE_SIZE == 4))
...
bool MpcNominalModel::f_x_unscentedUncertainty(const MpcNominalModel::model_state_t &x_k,
  ...
  if (x_k.x_k.size() != x_k.g_a_k_des_frame.size()) {
    ...
    for (int i = 0; i < size_test_vec; i++) {
      x_test.block(0, 2 * i, size_test_vec, 1) = x_test.block(0, 2 * size_test_vec, size_test_vec, 1)
          + std::pow(n_plus_kappa, 0.5) * S_k.block(0, i, size_test_vec, 1);
      x_test.block(0, 2 * i + 1, size_test_vec, 1) = x_test.block(0, 2 * size_test_vec, size_test_vec, 1)
          - std::pow(n_plus_kappa, 0.5) * S_k.block(0, i, size_test_vec, 1);
    }
