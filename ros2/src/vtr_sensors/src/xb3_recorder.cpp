...
Xb3Recorder::Xb3Recorder(const std::string &data_dir,
    ...
    : Node("xb3_recorder"),
      ...
      calib_writer_(data_dir, "xb3_calibration") {
  data_subscription_ = this->create_subscription<RigImages>(
      "xb3_images", 10,
      std::bind(&Xb3Recorder::imageCallback, this, std::placeholders::_1));
  calib_subscription_ = this->create_subscription<RigCalibration>(
      "xb3_calibration", 10,
      std::bind(&Xb3Recorder::calibCallback, this, std::placeholders::_1));
}
