...
bool MpcPath::loadGainScheduleConfigFile(std::string config_file_name) {
  try {
    ...
    nh_.param<double>(param_prefix_ + "gain_heading_error_ctrl_to_dir_sw", gain_schedule_tmp.dir_sw_heading_error_gain, 1.0);
    ...
    if (num_pos > 0) {
      if (num_neg > 0 && num_neg < num_pos) {
        LOG(ERROR) << "Path tracker speed schedule must have either equal number of pos/neg speed setpoints or only positive.";
        return false;
      }
    } else {
      LOG(ERROR) << "Path tracker speed schedule must have either equal number of pos/neg speed setpoints or only positive.";
    ...
    LOG(INFO) << "Loaded " << (int) gain_schedules_.size() << " gain schedules with " << (int) params_.speed_schedules.size() << " speeds";
  ...
  {
    LOG(ERROR) << "Bad yaml dereference exception : " << e.what() << ". Check to see if the pathtracker config file exists";
    std::cout << e.what() << "\n";
    return false;
  }
  catch(...)
  {
    LOG(ERROR) << "Unknown exception while loading the pathtracker config file. Check to see if the pathtracker config file exists";
    return false;
  }
}
...
bool MpcPath::loadCurvatureConfigFile(std::string config_file_name) {
  ...
  try{
    ...
    if ((int)params_.curvature_thresholds.size() * 2 != (int) params_.speed_schedules.size()){
      LOG(WARNING) << "Warning: loaded " << params_.curvature_thresholds.size() << " curvature thresholds but expecting " << params_.speed_schedules.size()/2;
    }
  ...
  {
    LOG(ERROR) << "Bad yaml dereference exception : " << e.what() << ". Check to see if the curvature config file exists";
    return false;
  }
  catch(...)
  {
    LOG(ERROR) << "Unknown exception while loading the curvature config file. Check to see if the curvature config file exists";
    return false;
  }
}
...
bool MpcPath::loadPathParams() {
  ...
  nh_.param<double>(param_prefix_ + "max_path_turn_radius_turn_on_spot_mode", params_.max_turn_radius_turnOnSpotMode, 0.9);
  ...
  nh_.param<double>(param_prefix_ + "max_tracking_error_rate_of_change", params_.max_tracking_error_rate_of_change, 0.3);
  ...
  nh_.param<double>(param_prefix_ + "reset_from_pause_slow_speed_zone_length_vertices", params_.reset_from_pause_slow_speed_zone_length_vertices, 2);
  // Verticeies with metric (?) tracking constraints
  nh_.param<std::vector<double>>(param_prefix_ + "list_of_constrained_vertices_from", list_of_constrained_vertices_from_, std::vector<double>());
  nh_.param<std::vector<double>>(param_prefix_ + "list_of_constrained_vertices_to", list_of_constrained_vertices_to_, std::vector<double>());
    ...
    if (n > (dir_sw_window_len - 1) ) {
      ...
      if(scheduled_ctrl_mode_[n - dir_sw_window_center] == VertexCtrlType::DIR_SW_POSE) {
        if (std::abs(dir_sw_window_sum) < 5){
          LOG(INFO) << "Invalid dir sw pose (" << n - dir_sw_window_center << ")....................with dir_sw indicator: " << std::abs(dir_sw_window_sum) << "/10" ;
          scheduled_ctrl_mode_[n - dir_sw_window_center] = VertexCtrlType::NORMAL;
        }
        else {
          LOG(INFO) << "Valid dir sw pose (" <<  n - dir_sw_window_center << ").....................with dir_sw indicator: " << std::abs(dir_sw_window_sum) << "/10" ;
        }
      }
    } // if (n > dir_sw_window_len - 1)
  } // for loop
  ...
  for (int n = 0; n <= N - 2; n ++) {
    ...
    if (n < N-2){
      if (scheduled_ctrl_mode_[n] == VertexCtrlType::TURN_ON_SPOT){
        if (scheduled_ctrl_mode_[n+1] != VertexCtrlType::TURN_ON_SPOT && scheduled_ctrl_mode_[n+2] == VertexCtrlType::TURN_ON_SPOT)
        {
...
void MpcPath::smoothScheduledSpeed() {
  ...
  for (int n = 0; n <= N-1; n++) {
    ...
    if (n == 0){
      ...
      window_sum = window_sum - std::abs(speed_profile_temp[window_start-1]) + std::abs(speed_profile_temp[window_end]);
    }
...
int MpcPath::findClosestSpeed(float v) {
  ...
  for (unsigned int i = 0; i < gain_schedules_.size(); i++) {
    ...
    {
      if (v <= gain_schedules_[i].target_linear_speed ||  // If we are less than or equal, take this gain schedule
          v *  gain_schedules_[i + 1].target_linear_speed < 0) // The next gain schedule is of opposite sign (in other words, we have gone through all the relevant ones)
      {
        min_indx = i;
        break;
      }
  ...
  if (min_indx == -1) {
    if (v<0)
      ...
      LOG(ERROR) << "You can't pass the path tracker a speed of zero because it doesn't know which gain schedule to use (i.e., a positive or negative)";
  }
  return min_indx;
}
