...
void MpcNominalModel::get_derivative_of_variance(MpcNominalModel::model_state_t & x_k, float d_t){
  ...
  x_k.dvar_x_dv(0,0) = -2*d_t*s_th*x_k.var_x_k(0,2);
  x_k.dvar_x_dv(0,1) = d_t*(c_th*x_k.var_x_k(0,2)-s_th*x_k.var_x_k(1,2));
  x_k.dvar_x_dv(1,0) = x_k.dvar_x_dv(0,1);
...
void MpcNominalModel::get_dF_dx(const MpcNominalModel::model_state_t & x_k, Eigen::MatrixXf & dF_dx, float d_t){
  ...
  dF_dx(1,1) = 1.0;
  dF_dx(2,2) = 1.0;
  dF_dx(0,2) = -d_t*sin(x_k.x_k(2))*x_k.command_k[0];
...
void MpcNominalModel::get_dF_du(const MpcNominalModel::model_state_t & , Eigen::MatrixXf & dF_du, Eigen::MatrixXf & dF_dukm1, float d_t){

  dF_du = Eigen::MatrixXf::Zero(STATE_SIZE, CONTROL_SIZE);
  if( MODEL_INCLUDES_VELOCITY == false){
    // v is fixed, so only with respect to omega
    dF_du(2,0) = d_t*K_OMEGA;
  } else {
    dF_du(3,0) = d_t*K_OMEGA;
  }

  // commented_out
  dF_dukm1 = Eigen::MatrixXf::Zero(STATE_SIZE, CONTROL_SIZE);
  if (MODEL_DEP_UKM1 == true){
    dF_du(2,0) = 0.0;
    dF_dukm1(2,0) = d_t*K_OMEGA;
  }
}

// Jacobian with respect to v for the Nominal Model
void MpcNominalModel::get_dF_dv(const MpcNominalModel::model_state_t & x_k, Eigen::MatrixXf & dF_dv, Eigen::MatrixXf & dF_dvkm1, float d_t){

  dF_dv = Eigen::MatrixXf::Zero(STATE_SIZE, CONTROL_SIZE);
  dF_dvkm1 = Eigen::MatrixXf::Zero(STATE_SIZE, CONTROL_SIZE);

  if (MODEL_DEP_UKM1 == true){
    dF_dvkm1(0,0) = (float) d_t*cos(x_k.x_k[2]);
    dF_dvkm1(1,0) = (float) d_t*sin(x_k.x_k[2]);
  } else {
    dF_dv(0,0) = (float) d_t*cos(x_k.x_k[2]);
    dF_dv(1,0) = (float) d_t*sin(x_k.x_k[2]);
  }
...
void MpcNominalModel::get_gdot(model_state_t & x_k, float d_t){
  ...
  /*** What?
    ...
    gx[0] = gradient of f_0:  gx[0] = (df_0/dx0, df_0/dx1, df_0/dx2), column vector
    gx[1] = gradient of f_1:  gx[1] = (df_1/dx0, df_1/dx1, df_1/dx2), column vector
    gx[2] = gradient of f_2:  gx[2] = (df_2/dx0, df_2/dx1, df_2/dx2), column vector
    N.B. df/dx_k^T = [gx[0], gx[1], gx[2]]
...
void MpcNominalModel::compute_dg_dx_and_dg_dxkm1(Eigen::MatrixXf & dG_dx, Eigen::MatrixXf & dG_dxkm1, const Eigen::MatrixXf & dg_da, const float & th_des, const float & d_t){
  ...
  // Going to get dg_dx    = dg_da * da_dx and
  //              dg_dxkm1 = dg_da * da_dxkm1
  // but fairly sparse matrix

  // Notes
  // e_l = sin(th_des)*(x_des - x) + cos(th_des)*(y_des - y)
  // e_h = th_des - th
  // omega_km1 = (th_k - th_km1) / d_t

  dG_dx = Eigen::MatrixXf::Zero(STATE_SIZE, STATE_SIZE);
  for (int i = 0; i < STATE_SIZE; i++){
    // dG_dx = dg_d(lat_error) * d(lat_error)_dx
    dG_dx(i,0) = d_t*dg_da(i,LAT_ERROR_K)  * (sin(th_des));

    //dG_dy = dg_d(lat_error) * d(lat_error)_dy
    dG_dx(i,1) = d_t*dg_da(i,LAT_ERROR_K)  * (-cos(th_des));

    if (MODEL_INCLUDES_VELOCITY == false){
      // dG_dth = dg_d(head_error) * d(head_error)_dth + dg_d(omega_km1) * d(omega_km1)_dth
      dG_dx(i,2) = d_t*(-dg_da(i,HEAD_ERROR_K) + dg_da(i,OMEGA_ACT_KM1) * (1/d_t));

    } else {
      // dG_dth = dg_d(head_error) * d(head_error)_dth
      dG_dx(i,2) = d_t*(-dg_da(i,HEAD_ERROR_K));
  ...
  dG_dxkm1 = Eigen::MatrixXf::Zero(STATE_SIZE, STATE_SIZE);
  for (int i = 0; i < STATE_SIZE; i++){

    if (MODEL_INCLUDES_VELOCITY == false){
      // dG_dth = dg_d(omega_km1) * d(omega_km1)_dth_km1
      dG_dxkm1(i,2) = d_t*dg_da(i,OMEGA_ACT_KM1) * (-1/d_t);
      ...
      // dG_dOmega_km1
      dG_dxkm1(i,3) = d_t*dg_da(i,OMEGA_ACT_KM1);
...
void MpcNominalModel::compute_dg_du_and_dg_dukm1(Eigen::MatrixXf & dG_du, Eigen::MatrixXf & dG_dukm1, const Eigen::MatrixXf & dg_da, const float & d_t){
  ...
  dG_du = Eigen::MatrixXf::Zero(STATE_SIZE,CONTROL_SIZE);
  dG_dukm1 = dG_du;

  for (int i = 0; i < STATE_SIZE; i++){

    dG_du(i,0)    = d_t*dg_da(i,OMEGA_CMD_K);
    dG_dukm1(i,0) = d_t*dg_da(i,OMEGA_CMD_KM1);
...
bool MpcNominalModel::computeDisturbancesForExperienceKm2(MpcNominalModel::experience_t & experience_km2, const MpcNominalModel::experience_t & experience_km1){
  ...
  bool result = compute_disturbance_from_state(experience_km2.gp_data.g_x_meas, x_km2, x_km1_act, d_t);
...
bool MpcNominalModel::computeDisturbancesForExperienceKm2SteamVel(MpcNominalModel::experience_t & experience_km2,
  ...
  // Rotate the body frame velocity from timestep km1 to frame km2 and compute model error
                                               ...
                                               float d_t)
{
  ...
  if (x_k.x_k.size() != x_k.g_a_k_des_frame.size()){
    ...
    x_test.block(size_x_k, 2*size_test_vec,size_x_k,1) = x_k.g_a_k_des_frame;

    // Compute the other six points
    for (int i=0; i<size_test_vec; i++){
      x_test.block(0,2*i,  size_test_vec,1) = x_test.block(0,2*size_test_vec,size_test_vec,1) + std::pow(n_plus_kappa,0.5)*S_k.block(0,i,size_test_vec,1);
      x_test.block(0,2*i+1,size_test_vec,1) = x_test.block(0,2*size_test_vec,size_test_vec,1) - std::pow(n_plus_kappa,0.5)*S_k.block(0,i,size_test_vec,1);
    }
    ...
    ut_output_mean = (1/n_plus_kappa)*(kappa*y_test.block(0,2*size_test_vec,size_x_k,1) + 0.5*sum_of_test);

    Eigen::MatrixXf y_0_m_mean(size_x_k,1);
    y_0_m_mean = y_test.block(0,2*size_test_vec,size_x_k,1) - ut_output_mean;
    ...
    x_kp1.var_x_k = 1/(n_plus_kappa)*(kappa*(y_0_m_mean*y_0_m_mean.transpose()) + 0.5*sum_cov);
...
void MpcNominalModel::computeInterpolatedDesiredPoseNew(const Eigen::MatrixXf & x_des_im1,
  ...
  float totalEx = std::abs(x_pred.tracking_error_km1(0)) + std::abs(x_pred.tracking_error_k(0));
...
void MpcNominalModel::computeDisturbanceDependancy(model_state_t & x_k,
  ...
  x_k.dist_along_path_k = computeDistAlongPath(nearest_path_length, x_k.tracking_error_k(0), x_k.command_km1[0]);
...
bool MpcNominalModel::generateWorstCaseTrajectories(MpcNominalModel::model_trajectory_t & x_sequence,
  ...
  // Worst-case disturbance applied to state x at time k expressed in x_des_0 frame
  ...
  for (int k = 0; k < numPoses; k++){
    ...
    Eigen::MatrixXf var_x_in_robot_frame = C_r_k * x_sequence[k].var_x_k * C_r_k.transpose();

    // Pick out disturbance 3*std deviations
    // Starts from 1 because we only want to transfer the uncertainty in the lateral position
    three_sigma_disturbance = Eigen::MatrixXf::Zero(STATE_SIZE,1);
    for (int i = 1; i < STATE_SIZE; i++){
      three_sigma_disturbance(i,0) = ((float) robust_control_sigma)*std::pow(var_x_in_robot_frame(i,i),0.5);
    }
...
void MpcNominalModel::initialize_state(MpcNominalModel::model_state_t & x_k){
  ...
  x_k.var_g_a_k_des_frame = 0.00001*Eigen::MatrixXf::Identity(STATE_SIZE,STATE_SIZE);
  ...
  x_k.Jx_gx.clear(); x_k.Jx_gx.resize(STATE_SIZE);
  x_k.Ju_gx.clear(); x_k.Ju_gx.resize(STATE_SIZE);
  x_k.Jv_gx.clear(); x_k.Jv_gx.resize(STATE_SIZE);
  x_k.Jx_gu.clear(); x_k.Jx_gu.resize(STATE_SIZE);
  x_k.Ju_gu.clear(); x_k.Ju_gu.resize(STATE_SIZE);
  x_k.Jv_gu.clear(); x_k.Jv_gu.resize(STATE_SIZE);
  x_k.Jx_gv.clear(); x_k.Jx_gv.resize(STATE_SIZE);
  x_k.Ju_gv.clear(); x_k.Ju_gv.resize(STATE_SIZE);
...
void MpcNominalModel::set_disturbance_model_zero(model_state_t & x_input){
  ...
  x_input.var_g_a_k = 0.00001*Eigen::MatrixXf::Identity(STATE_SIZE,STATE_SIZE);
  x_input.var_g_a_k_des_frame = 0.00001*Eigen::MatrixXf::Identity(STATE_SIZE,STATE_SIZE);
