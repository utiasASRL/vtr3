...
/** This reset occurs once per timestep **/
...
/** This computation occurs for each iteration of the solver **/
...
Eigen::MatrixXf compute_linear_abc(const Eigen::MatrixXf & point_1, const Eigen::MatrixXf & point_2, const Eigen::MatrixXf & des_point, const float & norm_dist){
  ...
  if (fabs(norm_dist) > 0){
    float norm_value = fabs(abc(0,0)*norm_point_lineFrame(0,0) + abc(1,0)*norm_point_lineFrame(1,0) + abc(2,0));
...
void MpcSolverXUopt::compute_constraints_V2(const local_path_t & local_path){
  ...
  float c_mult = 1.0f;
  if (opt_params.flg_en_robustMpcConstraints == false){
    c_mult = 0.0f;
  }
  ...
  for (int i=0; i < lookahead; i++){
    ...
    if (fabs(d_pose_k(0,0)) > d_max || i == lookahead-1 || flg_ovrlap_uncert == true){
      ...
      //if (flg_fill_rem_points == true || k_i >= lookahead-1){
      if (flg_fill_rem_points == true){
        break;
      } else {
        // Advance points
  ...
  for (int col=0; col<lookahead; col++){
    for (int row = 0; row<5; row++){
      if (std::isnan(x_lb_abc(row,col)) || std::isnan(x_ub_abc(row,col)) || fabs(x_lb_abc(row,col)) > 5000 || fabs(x_ub_abc(row,col)) > 5000){
        flg_nan = true;
      }
    }
  }
...
void MpcSolverXUopt::extract_grad_L(void){
  ...
  Eigen::MatrixXf delu_J = weight_Ru_diag.cwiseProduct(s_bar.block(u_offset_index,0,size_u_vec,1)            ) + dI_t_Rdu_dI*s_bar.block(u_offset_index,0,size_u_vec,1)+weight_u0;
  Eigen::MatrixXf delv_J = weight_Rv_diag.cwiseProduct(s_bar.block(v_offset_index,0,size_v_vec,1) - v_desired) + dI_t_Rdv_dI*s_bar.block(v_offset_index,0,size_v_vec,1)+weight_v0;

  if (delu_J.rows() != lookahead*size_u || delu_J.cols() != 1){
    LOG(WARNING) << "cwiseProduct failed to produce mtx with proper dimensions (path_tracker_mpc_solver_XUopt_implementation.hpp).";
  }
  ...
  for (int look_ahead_index = 0; look_ahead_index < lookahead; look_ahead_index++){
    ...
    if (opt_params.flg_en_mpcConstraints == true){
      // grad_x
      delx_zTg(state_index,0)   = s_bar(z_lb_index,  0)*x_lb_abc(0,look_ahead_index) + s_bar(z_ub_index,  0)*x_ub_abc(0,look_ahead_index);
      delx_zTg(state_index+1,0) = s_bar(z_lb_index,  0)*x_lb_abc(1,look_ahead_index) + s_bar(z_ub_index,  0)*x_ub_abc(1,look_ahead_index);
      delx_zTg(state_index+2,0) = s_bar(z_lb_index+1,0)*x_lb_abc(3,look_ahead_index) + s_bar(z_ub_index+1,0)*x_ub_abc(3,look_ahead_index);
    }
  }
  ...
  if (opt_params.flg_en_mpcConstraints == true){
    ...
    grad_L(z_lb_offset_index,0)      = -(-s_bar(0,0)       + 50.0 - s_bar(w_lb_offset_index,0));     // x_opt(0,0) > -50, hard-coded
    grad_L(z_ub_offset_index+z_K,0)  = -(-s_bar(state_K,0) + 50.0 - s_bar(w_ub_offset_index+z_K,0)); // x_opt(0,0) > -50, hard-coded
    ...
    for (int index = 0; index < lookahead; index++){
      ...
      // grad_z, lower bound
      grad_L(z_lb_offset_index+el_index_lb,0) = -(x_lb_abc(0,index)*x_act  + x_lb_abc(1,index)*y_act + x_lb_abc(2,index) - s_bar(w_lb_offset_index+el_index_lb,0));
      grad_L(z_lb_offset_index+eh_index_lb,0) = -(x_lb_abc(3,index)*th_act                           + x_lb_abc(4,index) - s_bar(w_lb_offset_index+eh_index_lb,0));

      // grad_z, upper bound
      grad_L(z_ub_offset_index+el_index_ub,0) = -(x_ub_abc(0,index)*x_act  + x_ub_abc(1,index)*y_act + x_ub_abc(2,index) - s_bar(w_ub_offset_index+el_index_ub,0));
      grad_L(z_ub_offset_index+eh_index_ub,0) = -(x_ub_abc(3,index)*th_act                           + x_ub_abc(4,index) - s_bar(w_ub_offset_index+eh_index_ub,0));
    ...
    if (USE_LOG_BARRIER_FUNC == true){
      ...
      grad_L.block(w_lb_offset_index,0, size_w_vec  ,1) = z_vec + 2*a_lin*w_vec.cwiseProduct(f_1) + a_lin*temp.cwiseProduct(df_1);
...
void MpcSolverXUopt::extract_J_grad_L(int iteration){
  ...
  int x_index, x_indexM1, x_indexM2, u_index, u_indexM1, v_index, v_indexM1, y_index; //, z_index_lb, z_index_ub, w_index_lb, w_index_ub;
  ...
  if (opt_params.flg_en_mpcConstraints){
    ...
    ds_opt.block(z_lb_offset_index,0,size_z_vec,1) = diag_A2.asDiagonal()*ds_opt.block(w_lb_offset_index,0,size_w_vec,1) + b_2;
  }
#else
  // Copy out ds_bar
  ...
  //  Compute step-size
  for (int index = 0; index < lookahead; index++){
    compute_limiting_step_size(step_size_, s_bar(u_offset_index+index,0), ds_opt(u_offset_index+index,0), -1.5f, 1.5f);
    compute_limiting_step_size(step_size_, s_bar(v_offset_index+index,0), ds_opt(v_offset_index+index,0), -1.5f, 1.5f);
  }
  float w_min, z_min;
  if (USE_LOG_BARRIER_FUNC == true){
    z_min = 0.0f;
  ...
  if (opt_params.flg_en_mpcConstraints){
    for(int index = 0; index < size_z_vec; index++){
      compute_limiting_step_size(step_size_, s_bar(w_lb_offset_index+index,0), ds_opt(w_lb_offset_index+index,0), w_min, 5000.0f);
      compute_limiting_step_size(step_size_, s_bar(z_lb_offset_index+index,0), ds_opt(z_lb_offset_index+index,0), z_min, 5000.0f);
    }
  }
  ...
  for (int i = 0; i < lookahead*CONTROL_SIZE; i++){
    ...
    if (sign_flip == true){
      ...
      sign_flip = true;
      LOG_EVERY_N(10,WARNING) << "MPC solution suggested direction switch when none was requested. Setting speed to slow.";
      //w_cmd = -w_cmd;
...
void MpcSolverXUopt::insert_mtx(mtx_triplet_list_t & triplet_list_out, const Eigen::MatrixXf & gradients_in, int & i, int & j, float sign /* = 1.0f */, bool transpose /* = false */){
...
void MpcSolverXUopt::insert_triplet_list(mtx_triplet_list_t & triplet_list_out, mtx_triplet_list_t & triplet_list_in, float & mult, int & i, int & j, float sign){
...
void MpcSolverXUopt::insert_triplet(mtx_triplet_list_t & triplet_list_out, const int & i, const int & j, float v_ij, float sign /* = 1.0f */){

  int index = s_bar.size()*i + j;
  int num_elements = s_bar.size()*s_bar.size();

  if (index > num_elements-1 || index < 0){
    LOG(INFO) << "Solver.insert_mtx:  Trying to add to (" << i << ", " << j << ") but mtx is of size (" << (int) s_bar.size() << ", " << (int) s_bar.size() << ").";
    LOG(INFO) << "Solver.insert_mtx:  Trying to add to element " << (int) index << " but num_elements = " << num_elements;
  }

  if (J_grad_L_mtx_entries_flg[index] == true){
    if (sign > 0){
...
void MpcSolverXUopt::insert_mtx(Eigen::MatrixXf & mtx, const Eigen::MatrixXf & gradients_in, int & i, int & j, float sign /* = 1.0f */, bool transpose /* = false */){
...
void MpcSolverXUopt::insert_triplet_list(Eigen::MatrixXf & mtx, mtx_triplet_list_t & triplet_list_in, float & mult, int & i, int & j, float sign){
...
void MpcSolverXUopt::insert_triplet(Eigen::MatrixXf & mtx, const int & i, const int & j, float v_ij, float sign /* = 1.0f */){
