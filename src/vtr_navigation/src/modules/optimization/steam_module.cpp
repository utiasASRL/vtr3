...
void SteamModule::run(QueryCache &qdata, MapCache &mdata,
  ...
  if (config_->use_T_q_m_prior && mdata.T_q_m_prior.is_valid() == true &&
      (*mdata.T_q_m_prior).covarianceSet() && (*mdata.T_q_m).covarianceSet()) {
    double prior_ct_sigma = sqrt((*mdata.T_q_m_prior).cov()(1, 1));
    double ct_sigma = sqrt((*mdata.T_q_m).cov()(1, 1));
