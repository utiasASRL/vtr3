...
unsigned ASRLStereoMatcherModule::matchFeatures(
  ...
  use_tight_pixel_thresh_ =
      mdata.T_q_m_prior.is_valid() &&
      sqrt(mdata.T_q_m_prior->cov()(0, 0)) < config_->tight_matching_x_sigma &&
      sqrt(mdata.T_q_m_prior->cov()(1, 1)) < config_->tight_matching_y_sigma &&
      sqrt(mdata.T_q_m_prior->cov()(5, 5)) <
  ...
  if (config_->prediction_method == se3 && mdata.T_q_m_prior.is_valid()) {
    // get the candidate transform given by a different function and transform
    // it to the camera frame
    auto T_q_m = (*qdata.T_sensor_vehicle) * (*mdata.T_q_m_prior) *
