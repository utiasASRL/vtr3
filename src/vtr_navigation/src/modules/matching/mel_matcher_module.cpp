...
void MelMatcherModule::run(QueryCache &qdata, MapCache &mdata,
  ...
  auto T_q_m_prior = T_s_v_q * (*mdata.T_q_m_prior) * T_s_v_m.inverse();
...
void MelMatcherModule::matchVertex(QueryCache &qdata, MapCache &mdata,
  ...
  for (uint32_t rig_idx = 0; rig_idx < query_landmarks.size(); ++rig_idx) {
    ...
    for (uint32_t channel_idx = 0;
         channel_idx < query_rig_landmarks.channels.size(); ++channel_idx) {
      const auto &map_channel_lm =
          map_rig_landmarks->channels().Get(channel_idx);
      asrl::vision::LandmarkId channel_id;
      channel_id.persistent =
          asrl::messages::copyPersistentId(vertex->persistentId());
...
void MelMatcherModule::matchChannel(
  ...
  asrl::vision::SimpleMatches channel_matches;
  ...
  for (uint32_t q_kp_idx = 0;
    ...
    if (query_matched_[q_kp_idx] == true) {
      ...
      if (best_match != -1) {
        ...
        asrl::vision::SimpleMatch map_match;
...
#pragma omp critical(updatematch)
        {
          ...
          asrl::vision::SimpleMatches::iterator mit = channel_matches.end();
          ...
          if (mit == channel_matches.end()) {
            ...
            const asrl::vision_msgs::Match &lm_track =
                map_channel_lm.matches().Get(best_match);
            map_matched_[asrl::messages::copyLandmarkId(lm_track.from())] =
                true;
...
void MelMatcherModule::matchChannelGPU(
  ...
  if (query_channel_lm.appearance.feat_type.impl ==
      asrl::vision::FeatureImpl::OPENCV_ORB) {
    // create the matcher
    cudabfmatcher =
    ...
    auto &map_descriptor_string = map_channel_lm.descriptors();
    float *map_descriptor = (float *)&map_descriptor_string.data()[0];
...
#else
  LOG(ERROR) << "You have asked MelMatcherModule to use CUDA GPU matching with "
                "OpenCV, but OpenCV wasn't built with that enabled!";
#endif
  ...
  for (unsigned qry_idx = 0; qry_idx < knnmatches.size(); ++qry_idx) {
    ...
    if (query_matched_[q_kp_idx] == true) {
      ...
      for (unsigned m_kp_idx = 0; m_kp_idx < knnmatches[qry_idx].size();
        ...
        const auto &lm_info_map = map_channel_lm.lm_info().Get(lm_idx);
        ...
        if (score <= best_score) {
          ...
          if (vo_obs.size() > 0) {
            track_length = vo_obs.Get(lm_idx);
          }
          ...
          if (potential_match(query_kp_info, lm_info_map, track_length,
                              ...
                              map_channel_lm.matches().Get(lm_idx))) {
            // yes! update it
            best_score = score;
            best_match = lm_idx;
...
int MelMatcherModule::matchQueryKeypoint(
  ...
  auto &query_kp = query_channel_obs.cameras[0].points[q_kp_idx];
  auto &query_point = query_channel_lm.points.col(q_kp_idx);
  float *query_descriptor = (float *)&query_channel_lm.appearance.descriptors
  ...
  for (int lm_idx = 0; lm_idx < map_channel_lm.points().size(); ++lm_idx) {
    const auto &lm_info_map = map_channel_lm.lm_info().Get(lm_idx);
    ...
    if (potential_match(query_kp_info, lm_info_map, track_length, query_kp,
      ...
      float *map_descriptor =
          (float *)&map_descriptor_string.data()[step_size * lm_idx];
      double score = asrl::vision::ASRLFeatureMatcher::distance(
...
inline bool MelMatcherModule::potential_match(
    ...
    const double &map_depth, const asrl::vision_msgs::Match &lm_track) {
  ...
  if (config_->screen_matched_landmarks) {
    ...
    for (int track_idx = 0; track_idx < lm_track.to().size(); ++track_idx) {
      if (map_matched_.find(asrl::messages::copyLandmarkId(
              lm_track.to().Get(track_idx))) != map_matched_.end()) {
        previously_matched = true;
        continue;
      }
    }

    if (previously_matched == true) {
      return false;
    }
  }
