...
unsigned vertexDistance(VertexId a, VertexId b, const Graph &graph) {
  ...
  for (RunId rid = a.majorId(); rid < b.majorId(); ++rid) {
    try {
      distance += graph.run(rid)->vertices().size();
    } catch (...) {
    }
  }
...
State decayState(State state, double decay_factor, const VertexId &new_vid,
                 const Graph &graph) {
  // Compute the decay we need for state.vid -> new_vid
  unsigned distance = vertexDistance(state.vid, new_vid, graph);
  distance =
      std::max(1u, distance);  // Guarantee at least 1 decay (reprocessing)
...
State dropStale(State state, double thresh, bool verbose = false) {
  ...
  for (auto rit = state.runs.begin(); rit != state.runs.end(); /*++ inside*/) {
    ...
    LOG_IF(stale && verbose, INFO) << "Removing stale run " << rit->first
                                   << " from the collaborative state.";
    if (stale)
      rit = state.runs.erase(rit);
    else
      ++rit;
  }
  return state;
...
unsigned landmarkCounts(const asrl::vision_msgs::RigCounts &rig_msg) {
  unsigned count = 0;
  for (const asrl::vision_msgs::ChannelCount &channel_msg :
       rig_msg.channels()) {
...
/// Treats experiences as 'users' and landmark-match connected components as
/// 'items' Based on: http://dl.acm.org/citation.cfm?id=1864729 Liu, N. N.,
/// Zhao, Min, et al., Online Evolutionary Collaborative Filtering Also
/// consider:
/// http://courses.ischool.berkeley.edu/i290-dm/s11/SECURE/a1-koren.pdf 0. Get
/// the local match history from the localization submap,
...
void CollaborativeLandmarksModule::run(
  ...
  // 0. Visit every vertex in the local submap (we count on submap
  // extraction...)
  // ----------------------------------------------------------------------------
  for (auto vit = submap.begin(); vit != submap.end(); ++vit) {
    ...
    for (const std::string &rig_name : rig_names) {
      ...
      asrl::common::timing::SimpleTimer load_timer;
      auto count_msg = vi.retrieveKeyframeData<asrl::vision_msgs::RigCounts>(
          "/" + rig_name + "/landmarks/counts");
      load_time += load_timer.elapsedMs();
      //      LOG_IF(!count_msg, ERROR) << "No landmark count messages for " <<
      //      vi.id() << " " << rig_name;
      if (!count_msg) continue;
      lm_counts_[vi.id().majorId()] += landmarkCounts(*count_msg);

      if (submap_ids_.count(vi.id()))
        continue;  // we already have this vertex in the cache from last time
      // update the landmark matches to other runs from this vertex
      load_timer.reset();
      auto matches_msg = vi.retrieveKeyframeData<asrl::vision_msgs::Matches>(
          "/" + rig_name + "/landmarks/matches");
      load_time += load_timer.elapsedMs();
      if (!matches_msg) continue;  // privileged runs don't have matches
      for (const asrl::vision_msgs::Match &match : matches_msg->matches()) {
        if (!match.to_size()) continue;  // shouldn't happen, but just in case
        const asrl::vision_msgs::FeatureId &to_fid = match.to(0);
        try {
  ...
  // 1. Go through the live landmarks, and follow their matches into the match
  // map
  // -----------------------------------------------------------------------------
  ...
  state_ =
      decayState(std::move(state_), config_->similarity_decay, live_id, *graph);
  // Remove any stale runs from the state map
  ...
  for (const auto &rpair : state_vo.runs) {
    if (rpair.second.P < 1.)
      continue;  // not even 1 match... nah (also block legacy runs)
    if (!available_runs.count(rpair.first))
      continue;  // No vertices available, don't bother
  ...
  bool in_the_loop = config_->in_the_loop && !distance_rids.empty();
  ...
  if (config_->recommend_landmarks && in_the_loop) {
    ...
    for (const MatchCache::ValuePack &cm : match_cache_.connected()) {
      ...
      // Find the score for this landmark (sum of similarities of connected
      // runs)
      float score = 0.f;
      ...
      for (const vtr::vision::LandmarkId &lid : cm.keys) {
        ...
        if (recommended.count(vid.majorId())) scored_lms_.emplace(score, lid);
      }
    }
  }
  ...
  asrl::status_msgs::ExpRecogStatus &status_msg = status_msg_to_save_;
  status_msg.Clear();
  ...
  status_msg.set_algorithm(asrl::status_msgs::ExpRecogStatus_Algorithm_COLLAB);
  ...
  for (const RunId &rid : recommended) status_msg.add_recommended_ids(rid);
...
void CollaborativeLandmarksModule::updateGraph(
  ...
  // we need to take all of the landmark matches and update connected store them
  // in the new landmarks
  ...
  // 2. Update the state using the ransac-confirmed matches (forget the vo
  // matches)
  // ------------------------------------------------------------------------------
  ...
  for (const auto &cnt : lm_counts_) {
    ...
    if (tried_run) {
      ...
      unsigned attempts =
          match_cnt + unsigned((lm_cnt - match_cnt) * pessimism);
  ...
  if (!scored_lms_.empty()) {
    ...
    LOG(INFO) << "CF-lm: range: " << scored_lms_.begin()->first << "-"
              << scored_lms_.rbegin()->first << " ["
              << sum_all / migrated_landmark_ids.size() << "] "
              << (double(scored_lms_.size()) / migrated_landmark_ids.size())
              << " % "
              << "match: " << scores.begin()->first << "-"
              << scores.rbegin()->first << " [" << (sum_match / matches.size())
  ...
  status_msg_to_save_.set_computation_time_ms(
      status_msg_to_save_.computation_time_ms() + update_time);
