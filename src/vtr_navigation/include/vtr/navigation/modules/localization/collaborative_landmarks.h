...
class MatchCache {
 public:
  ...
  struct Merger {
    Value operator()(Value lhs, Value &&rhs) { return lhs += std::move(rhs); }
  };
 ...
 public:
  ...
  std::pair<iterator, bool> insert(const ValuePack &vp) {
    return emplace(ValuePack(vp));
  }
...
class CollaborativeLandmarksModule : public BaseModule {
 public:
  ...
  struct State {
    VertexId vid =
        VertexId::Invalid();  ///< Represents the state at this vertex
    double Qlive = 0.;
    struct RunState {
      double Q = 0.;
      double P = 0.;
  ...
  struct MultiMatch {
    RunId rid;
    std::unordered_set<asrl::vision_msgs::FeatureId> down_match;
    std::unordered_set<asrl::vision_msgs::FeatureId> up_match;
    std::unordered_set<RunId> down_rid;
    std::unordered_set<RunId> up_rid;
  };
  typedef std::unordered_map<asrl::vision_msgs::FeatureId, MultiMatch> MatchMap;
