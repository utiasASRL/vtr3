...
Image RGB2Grayscale(const Image &src){
  Image dst;
  cv::cvtColor(src.data,dst.data,cv::COLOR_BGR2GRAY); // vtr3 change: opencv 4+
  dst.name = src.name;
  return dst;
}
...
ChannelImages RGB2ColorConstant(const ChannelImages & src, float alpha, bool histogram_equalization) {
  ...
  // start the conversion job in parallel for each camera in the channel.
  std::list<std::future<Image>> futures;
  auto func = static_cast<Image(*)(const Image&,float&,bool)>(&RGB2ColorConstant);
  for(const auto& image : src.cameras) {
    futures.emplace_back(std::async(std::launch::async,func,std::ref(image),std::ref(alpha),histogram_equalization));
  }
...
ChannelImages Gray2Undistorted(const ChannelImages & src, const CameraIntrinsics & intrinsics, const CameraDistortions & dists) {
  ...
  // ...well, that was the plan. It turns out cv::undistort is not thread-safe and corrupts images if called
  // in parallel. Therefore the images are undistorted here in a non parallel fashion. The code to undistort
  // in parallel is left here for posterity and such time as the OpenCV library function is fixed.
  //std::list<std::future<void>> futures;
  //auto func = static_cast<void(*)(cv::InputArray&, cv::OutputArray&, cv::InputArray&, cv::InputArray&, cv::InputArray&)>(&cv::undistort);
  ...
  for(const auto& image : src.cameras) {
    ...
    cv::Matx33d intrinsic(intrinsics[ii](0,0), intrinsics[ii](0,1), intrinsics[ii](0,2),
        intrinsics[ii](1,0), intrinsics[ii](1,1), intrinsics[ii](1,2),
        intrinsics[ii](2,0), intrinsics[ii](2,1), intrinsics[ii](2,2));
    cv::Vec<double,5> dist(dists[ii](0), dists[ii](1), dists[ii](2), dists[ii](3), dists[ii](4));

    // replace the parallel method with a direct call
    //futures.emplace_back(std::async(func,image.data,dstimg.data, intrinsic, dist, cv::noArray()));
