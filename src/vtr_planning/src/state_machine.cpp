...
/// @brief Performs state transitions in a loop until a stable state is reached
...
void BaseState::processGoals(Tactic*,
                             ...
                             const Event& event) {
  // The reset flag stops us from triggering a success callback on an Abort or a
  // NewGoal with Idle target
  bool reset = false;
  {
    ...
    // Check if there is another goal we must transition through first to get to
    // the target goal. We don't need to check if the target was changed, as
    // goal->nextStep(goal) always returns nullptr.
...
auto BaseState::nextStep(const BaseState* newState) const -> Ptr {
  ...
  if (Idle::IsType(newState) || Learn::InChain(newState) ||
    ...
    //    - A transition that should have been handled lower in the tree wasn't
  } else {
    std::stringstream ss;
