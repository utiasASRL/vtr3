...
#include <asrl/planning/Event.hpp>
...
class BaseState {
 public:
  ...
  typedef StateMachineInterface Tactic;
  typedef boost::upgrade_lock<boost::shared_mutex> UpgradableLockGuard;

  typedef BaseState Base;
  typedef std::shared_ptr<std::list<Ptr> > GoalStackPtr;
...
class StateMachine {
 public:
  ...
  typedef boost::upgrade_lock<boost::shared_mutex> UpgradableLockGuard;
  typedef boost::upgrade_to_unique_lock<boost::shared_mutex> UpgradedLockGuard;
 ...
 protected:
  ...
  /// @detail This flag is set to true whenever ::Teach is entered.  It is also
  /// true on construction as the first
  ///         repeat in any period of operation must generate a new run.
